...
--
-- 산출물  C:/snatcher/dump/vram_writer_<시각>.tsv

local MEM, CPU = emu.memType.pceMemory, emu.cpuType.pce
local VRAM = emu.memType.pceVideoRam

local STAMP = os.date('%Y%m%d_%H%M%S')
local PATH  = 'C:/snatcher/dump/vram_writer_' .. STAMP .. '.tsv'
...
local BYTE_LO, BYTE_HI = TILE_FIRST * 32, TILE_LAST * 32 + 31   -- $2220-$319F
...
end, emu.callbackType.write, BYTE_LO, BYTE_HI, CPU, VRAM)
...
local function report()
  ...
  say('')
  say(('=== %d 번째 업로드 끝  f%d  --  VRAM 쓰기 %d 회 · PC %d 종')
    :format(upload, frame, hits, #pcs))
  if #pcs == 0 then
    say('  ★PC 를 하나도 못 잡았다 -- 에뮬레이터가 VRAM 쓰기에 PC 를 안 준다')
    out:write(('%d\t%d\t\t0\n'):format(upload, frame))
  end
...
say('PROBE GFX 0.1.6-vram-writer armed -- 순수 관측')
