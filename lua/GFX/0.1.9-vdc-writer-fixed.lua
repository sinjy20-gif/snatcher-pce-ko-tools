...
--
-- 산출물  C:/snatcher/dump/vdc_writer2_<시각>.tsv

local MEM, CPU = emu.memType.pceMemory, emu.cpuType.pce
local VRAM = emu.memType.pceVideoRam

local STAMP = os.date('%Y%m%d_%H%M%S')
local PATH  = 'C:/snatcher/dump/vdc_writer2_' .. STAMP .. '.tsv'
...
local function report()
  ...
  for i = 1, math.min(#pcs, 10) do
    local pc = pcs[i][1]
    local mark = ''
    if mawr_lo[pc] and mawr_lo[pc] <= WORD_HI and mawr_hi[pc] >= WORD_LO then
      mark = '  ★우리 구간을 건드린다'
    end
  ...
  if #pcs == 0 then say('  ★그래도 못 잡았다') end
  out:flush()
...
say('PROBE GFX 0.1.9-vdc-writer-fixed armed -- 순수 관측')
