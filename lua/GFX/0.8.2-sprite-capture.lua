-- GFX 0.8.2 -- 장면이 바뀔 때마다 VRAM 을 따로 뜬다  (0.8.0 의 구멍을 막은 판)
--
-- 0.8.0 이 틀렸던 것
-- ------------------
...
local function dump(reason)
  ...
  emu.log(string.format("SPRITE CAPTURE 0.8.2 [%s] %s · 프레임 %d · 스프라이트 %d 종 · 장면 %d 개",
...
emu.log("GFX SPRITE CAPTURE 0.8.2  [" .. LABEL .. "]  읽기 전용")
emu.log("  ★ 장면이 바뀔 때마다 VRAM/CRAM 을 따로 뜬다 -- 주행 한 번으로 전부 건진다")
emu.log("  오프닝 시작 전에 올리고 끝까지 본 뒤 F")
emu.log("  -> " .. prefix .. "_*")
