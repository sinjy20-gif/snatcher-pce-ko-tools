-- SNATCHER title menu sprite capture 0.7.3
--
-- 0.7.2 에서 바뀐 것 (2026-09-15)
--   * ★ VRAM **전체 64 KB** 를 뜬다.  디스크에서 원본 압축 블록을 찾으려면
--     `gfx_locate_blocks.find_at` 가 블록 **전부**를 VRAM 과 대조하는데,
--     0.7.2 는 스프라이트 7 조각(512 B 씩)만 떠서 그 대조를 할 수 없었다.
--   * ★ `$6E00` 블록 추가.  화면에 살아 있는 sprite 1(x=176, 16x16)이
--     `초めから` 줄의 꼬리인데 0.7.2 목록에 빠져 있었다.
--   * 스프라이트 패턴 형식 확정: **little-endian 워드 · 행 우선 셀**
--     (0.7.2 덤프를 네 조합으로 그려 확인 -- 「初めから」「<원문 8자>」)
--
...
local BLOCKS = {
  ...
  { id = "new_4",  word = 0x6E00, words = 0x040, text = "처음부터: sprite 1, 16x16  ★0.7.3 추가" },
...
local function dump_all(reason)
  ...
  write_binary(prefix .. "_vram_full.bin", VRAM, 0, 0x10000)   -- ★ 전체 VRAM
