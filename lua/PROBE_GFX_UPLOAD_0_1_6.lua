-- PROBE_GFX_UPLOAD 0.1.6 -- VDC 레지스터까지 같이 뜬다 (2026-08-25)
--
-- 0.1.5 에서 또 걸린 것
-- ---------------------
...
local BASE  = "C:\\snatcher\\dump\\gfx_upload_0_1_6_" .. stamp
...
emu.addEventCallback(function()
  ...
  if not dumped and fills > 1000 and lastFill > 0 and frames - lastFill > 20 then
    dumped = true
    dump_all("settled")
    emu.log("GFX 0.1.6 -- 채우기가 멎어 소스를 떴다")
  end

  if frames % 1800 == 0 then
    emu.log(string.format("GFX 0.1.6 -- 버퍼채움 %d · 소스읽기 %d · 떴나 %s",
...
emu.addEventCallback(function()
  ...
  if file == nil then return end
  file:write("PROBE_GFX_UPLOAD 0.1.6  " .. stamp .. "\n")
  ...
  file:close()
  emu.log("GFX 0.1.6 보고서 -> " .. BASE .. "_report.txt")
end, emu.eventType.scriptEnded)

emu.log("PROBE_GFX_UPLOAD 0.1.6 -- CD RAM 창($8000-$DFFF)을 통째로 뜬다")
