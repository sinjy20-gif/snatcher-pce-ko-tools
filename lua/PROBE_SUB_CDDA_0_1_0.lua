-- PROBE_SUB_CDDA 0.1.0 -- CD-DA LBA 구간 자막 POC. ADPCM에는 반응하지 않는다.
--
-- 왜 바꿨나
-- ---------
...
--
-- 0.1.10 에서 온 것: 열쇠는 **sector + 끝 주소 + 재생률**
--
-- 0.1.3 이 무엇을 보여줬나
-- ------------------------
...
local function u24(o) return u8(o) | (u8(o + 1) << 8) | (u8(o + 2) << 16) end
...
local function parse_pack()
  ...
  P.version = u16(4)
  if P.version ~= 5 then
    say('★ 이 Lua 는 sector 키 팩 v5 전용이다 (현재 v' .. tostring(P.version) .. ')')
    return false
  end
...
local function find_parts(sector, end_addr, rate)
  local out = {}
  for i = 0, P.adpcm_count - 1 do
    local at = P.adpcm_off + i * P.adpcm_stride
    local item_sector = u8(at) | (u8(at + 1) << 8) | (u8(at + 2) << 16)
    if item_sector == sector and u16(at + 3) == end_addr and u8(at + 5) == rate then
...
-- CD-DA 색인은 lba_from 순이다. 끝은 exclusive 이므로 이분 탐색 뒤 한 번만 확인한다.
local function find_cdda(sector)
  local lo, hi, answer = 0, P.cdda_count - 1, nil
  while lo <= hi do
    local mid = math.floor((lo + hi) / 2)
    local at = P.cdda_off + mid * P.cdda_stride
    if u24(at) <= sector then answer = at; lo = mid + 1 else hi = mid - 1 end
  end
  if not answer then return nil end
  local to = u24(answer + 3)
  if sector >= to then return nil end
  return { from = u24(answer), to = to, rec = u24(answer + 6) }
end
...
local checked = false
local last_cdda_sector, cdda_last_advance = nil, -1000
local cdda_record = nil
...
local function flush()
  local f = io.open(string.format('C:/snatcher/dump/probe_sub_cdda_0_1_0_%s.tsv',
...
if ready then
  emu.log(string.format('팩 버전 %d · CD-DA 구간 %d · 글리프 %d 자',
                        P.version, P.cdda_count, P.glyph_count))
end
...
emu.addEventCallback(function()
  ...
  local advancing = type(sector) == 'number' and type(last_cdda_sector) == 'number'
      and sector > last_cdda_sector and sector - last_cdda_sector < 8
  if advancing then cdda_last_advance = frames end
  if type(sector) == 'number' then last_cdda_sector = sector end

  if frames - cdda_last_advance <= CDDA_STOP_GRACE and type(sector) == 'number' then
    local hit = find_cdda(sector)
    if hit then
      local key = string.format('%06X-%06X@%d', hit.from, hit.to, hit.rec)
      if key ~= cdda_record then
        local record = read_record(hit.rec)
        stage(record)
        cdda_record = key
        shown = shown + 1
        check_at, check_n = frames + 8, n_cells(record)
        say(string.format('[%d] CDDA LBA %06X  %06X-%06X  y=%d  "%s"',
              frames, sector, hit.from, hit.to, record.y, record.text))
      end
    elseif armed then
      disarm(); cdda_record = nil
      say(string.format('[%d] CDDA 자막 구간 밖 -- VRAM %d 워드 되돌림', frames, VRAM_WORDS))
    end
  elseif armed then
    disarm(); cdda_record = nil
    say(string.format('[%d] CDDA 정지 -- VRAM %d 워드 되돌림', frames, VRAM_WORDS))
  end
...
emu.log('PROBE_SUB_CDDA 0.1.0 loaded')
emu.log('  ★ build/patch/subtitle_resident/ 의 디스크로 켤 것')
emu.log('  CD-DA sector가 전진하는 동안만 팩의 LBA 구간을 조회한다 (ADPCM은 무시)')
