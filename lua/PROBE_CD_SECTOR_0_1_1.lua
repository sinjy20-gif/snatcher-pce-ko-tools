-- PROBE_CD_SECTOR 0.1.1 -- ADPCM 을 싣는 BIOS 호출을 찾는다 (2026-08-26)
--
-- 0.1.0 이 답한 것
-- ---------------
...
local PATH = "C:\\snatcher\\dump\\cd_sector_0_1_1_" .. stamp .. ".txt"
...
local function on_call(entry)
  return function()
    ...
    if #calls > 60 then table.remove(calls, 1) end
  end
end
...
emu.addEventCallback(function()
  ...
  if playing and not was_playing then
    ...
    local found = false
    for i = #calls, math.max(1, #calls - 12), -1 do
      local c = calls[i]
      local where = {}
...
emu.log("PROBE_CD_SECTOR 0.1.1 -- BIOS 진입점 32 개를 전부 걸었다")
