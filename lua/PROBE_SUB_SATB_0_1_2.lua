-- PROBE_SUB_SATB 0.1.2 -- 자막이 왜 둘로 보이나 (2026-08-26)
--
-- 증상
-- ----
...
--   3) Stop  ->  dump/sub_satb_0_1_2_<시각>.tsv

local MEM  = emu.memType.pceMemory
local VRAM = emu.memType.pceVideoRam

-- ★ Mesen 의 pceVideoRam 은 **바이트 주소**다.  워드로 읽으면 엉뚱한 데를 본다.
--   어제 프로브(PROBE_SUBTITLE_FRAGMENT_ALLOCATOR)가 쓰던 값과 맞췄다:
--       SATB 바이트 $2000 · 한 항목 8 B · 패턴은 +4
local SATB = 0x2000           -- VRAM **바이트**
local STRIDE = 8
local SPRITES = 64
local GLYPH_VRAM = 0x7900     -- 글리프를 올리는 자리 (엔진 기본값)

local stamp = os.date("%Y%m%d_%H%M%S")
local PATH = "C:\\snatcher\\dump\\sub_satb_0_1_2_" .. stamp .. ".tsv"
...
-- 패턴 번호 -> VRAM 워드 주소.  스프라이트 한 장이 64 워드라 pattern << 5 다.
-- (어제 프로브의 `base >> 5` 와 같은 규약.  앞서 쓴 `(p>>1)<<6` 은 홀수에서 어긋난다.)
local function pattern_word(pattern) return (pattern << 5) & 0xFFFF end
local function is_ours(pattern)
  local word = pattern_word(pattern)
  return word >= GLYPH_VRAM and word < GLYPH_VRAM + 0x4C0   -- 19 자 x $40
end
...
emu.addEventCallback(function()
  ...
  for i = 0, SPRITES - 1 do
    ...
    local function w(o) return (emu.read(at + o, VRAM) or 0) | ((emu.read(at + o + 1, VRAM) or 0) << 8) end
    local y, x, pattern, attr = w(0), w(2), w(4), w(6)
    if pattern ~= 0 then
      local word = pattern_word(pattern)
...
emu.log("PROBE_SUB_SATB 0.1.2 -- 자막이 뜨는 동안 SATB 를 훑는다")
