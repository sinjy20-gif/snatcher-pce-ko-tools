-- lua/COST/0.3.0-clipend-vs-template.lua   (옛 이름 PROBE_ARM_TRANSPORT_0_2_2.lua)
-- ★판번호는 COST 폴더 기준으로 다시 매겼다 (2026-09-05, 소유자 요청).
--   덤프 경로도 같이 바꿨다 -- 로그와 코드가 어긋나면 로그를 못 읽는다.
--   덤프: snatcher_tool/logs/cost_v030.tsv
--
-- PROBE_ARM_TRANSPORT 0.2.2 -- 671 B 재적재가 정말 필요한가
--
-- ★ 0.2.1 고침.  0.2.0 은 B 표본을 하나도 못 떴다 -- 발견이 아니라 결함이었다.
--   STATE 는 1 -> 2 -> 0 으로 간다 (0.1.0 이 이미 state 2 를 찍었다).
--   그런데 0.2.0 은 `1 -> 0` 만 봤으므로 클립 끝이 영영 안 잡혔다.
--   이제 0 이 아닌 값에서 0 으로 가는 것을 본다.
--
-- 0.1.0 이 답한 것 (실측)
-- ----------------------
...
--
--   dump  snatcher_tool/logs/cost_v030.tsv   ★프로브와 같은 판번호

local OUT = "C:/snatcher/snatcher_tool/logs/cost_v030.tsv"
...
emu.addMemoryCallback(function(addr, value)
  if value == 1 and last_state ~= 1 then
    arms = arms + 1
    pending = "A"
  elseif value == 0 and last_state > 0 then
    ends = ends + 1
    pending = "B"
  end
  last_state = value
...
emu.addEventCallback(function()
  ...
  elseif pending == "B" then
    -- ★ 비교 대상은 템플릿이다 (A 가 아니다).
    --   소유자 지적: 대사가 이어지면 패치 바이트까지 같은 값이라 A 와의 차이는
    --   늘 0 에 가깝다 -- 그걸로는 "복사가 필요했나" 를 못 가른다.
    --   진짜 질문은 **다음 arm 직전에 슬롯이 이미 템플릿이었나** 이고,
    --   클립이 끝나면 다음 arm 까지 아무도 안 건드리므로 여기서 뜨면 된다.
    local now = grab(AC_ENGINE)
    local dt = diff(now, grab(AC_TEMPL))
    say(("  f%-8d [B] 클립 끝 -- ★템플릿과 다른 바이트 %d개  %s")
        :format(frame, #dt, brief(dt)))
    verdicts[#verdicts + 1] = { frame = frame, kind = "B", n = #dt, list = dt }
    pending = nil
  end
...
emu.addEventCallback(function()
  ...
  elseif worst <= 16 then
    say(("-> 클립이 끝난 시점의 슬롯이 템플릿과 최대 %d 바이트만 다르다")
        :format(worst))
    say("   즉 다음 arm 의 671 B 복사는 **이미 맞는 값을 다시 쓰는 것**이다.")
    say("   그 몇 바이트만 되돌리면 되므로 23.6 줄이 사라진다.")
    say("   ⚠ 단 CD-DA 가 중간에 끼면 슬롯이 통째로 바뀐다 -- 그때는 복사가 맞다")
  else
    say(("-> 슬롯이 템플릿과 %d 바이트나 다르다.  복사가 필요한 게 맞다")
        :format(worst))
    say("   (렌더러 자기수정이거나, 중간에 CD-DA 가 슬롯을 가져간 것)")
  end
  say(("dump  %s"):format(OUT))
end, emu.eventType.scriptEnded)

say("PROBE_ARM_TRANSPORT 0.2.2 -- 세고, arm 직후/클립끝 두 번 뜬다")
