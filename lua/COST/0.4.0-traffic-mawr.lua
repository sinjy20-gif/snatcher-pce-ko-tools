-- lua/COST/0.4.0-traffic-mawr.lua   (옛 이름 PROBE_SUBTITLE_TRAFFIC_0_3_1.lua)
-- ★판번호는 COST 폴더 기준으로 다시 매겼다 (2026-09-05, 소유자 요청).
--   덤프 경로도 같이 바꿨다 -- 로그와 코드가 어긋나면 로그를 못 읽는다.
--   덤프: snatcher_tool/logs/cost_v040.tsv
--
-- PROBE_SUBTITLE_TRAFFIC 0.3.1 -- 자막 때문에 나가는 비용만 골라 센다
--
...
--
--   dump  snatcher_tool/logs/cost_v040.tsv   ★프로브와 같은 판번호

local OUT = "C:/snatcher/snatcher_tool/logs/cost_v040.tsv"
...
emu.addEventCallback(function()
  ...
  if ours > 0 or acw > 0 then
    ...
    if busy <= 40 then
      ...
      say(("  f%-7d %-9s 자막VRAM %5d  AC쓰기 %5d  (SATB %3d · 게임 %4d)  %6d cyc %5.1f줄  시작줄 %4d%s")
...
say("PROBE_SUBTITLE_TRAFFIC 0.3.1 -- MAWR 을 따라가 우리 것만 센다")
