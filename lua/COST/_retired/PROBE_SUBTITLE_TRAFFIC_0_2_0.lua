-- PROBE_SUBTITLE_TRAFFIC 0.2.0 -- 자막이 나올 때 복사가 몇 번, 얼마나 일어나나
--
-- 질문 (소유자, 2026-09-05)
-- ------------------------
...
--
--   dump  snatcher_tool/logs/subtitle_traffic_v020.tsv   ★프로브와 같은 판번호

local OUT = "C:/snatcher/snatcher_tool/logs/subtitle_traffic_v020.tsv"
...
-- 스캔라인 키 이름은 코어 판마다 다르다.  한 번만 찾아 기억한다 (0.4.70/0.4.96 방식).
local LINE_KEY
...
-- ★ getState 는 **프레임당 최대 한 번**만 부른다.  콜백 안에서 매번 부르면
--   PROBE_ADPCM_BUDGET 0.1.0 처럼 에뮬이 선다.  그래서 그 프레임의 첫 쓰기에서만
--   찍는다 -- 스파이크가 시작된 자리를 알면 충분하다.
local burst_line = -1

emu.addMemoryCallback(function() rd = rd + 1 end,
  emu.callbackType.read, AC_READ, AC_READ, emu.cpuType.pce, cpu)
emu.addMemoryCallback(function()
  if wr == 0 then burst_line = scanline() end
  wr = wr + 1
...
emu.addEventCallback(function()
  ...
  if total >= QUIET or vd >= 64 then
    ...
    rows[#rows + 1] = { f = frame, rd = rd, wr = wr, vd = vd,
                        kind = kind, arms = arms, line = burst_line }
    if busy <= 40 then
      say(("  f%-7d %-10s AC읽기 %5d  AC쓰기 %5d  VRAM쓰기 %5d  시작줄 %4d %s")
          :format(frame, kind, rd, wr, vd, burst_line,
                  (burst_line >= 0 and burst_line < 242) and "★표시구간" or ""))
    end
  end
  rd, wr, vd = 0, 0, 0
  burst_line = -1
...
say("PROBE_SUBTITLE_TRAFFIC 0.2.0 -- AC 읽기/쓰기 + VRAM 쓰기를 프레임마다")
