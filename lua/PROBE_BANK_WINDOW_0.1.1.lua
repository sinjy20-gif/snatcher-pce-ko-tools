-- PROBE 뱅크창 0.1.1 -- **뱅크가 바뀐 동안 인터럽트가 들어오는가**만 본다
--
-- 증명할 명제 (2026-08-19, 조이 디비전 쇼핑하기 진행불가)
-- ---------------------------------------------------------------------------
-- $7F88 트램펄린은 헬퍼를 부르려고 $A000-$BFFF 를 뱅크 $69 로 갈아끼운다:
--
--     7F88  PHP / SEI       ★ 0.4.5.2 에서 추가된 보호
--     7F8A  TMA #$20        현재 뱅크 저장
--     7F8A  PHA
--     7F8B  LDA #$69
...
--   출력: C:\snatcher\dump\probe_bank_window_0_1_1.tsv

local OUT = "C:\\snatcher\\dump\\probe_bank_window_0_1_1.tsv"
local mem = emu.memType.pceMemory

local WINDOW_OPEN  = 0x7F94   -- TAM 직후 (뱅크 바뀐 상태)
local WINDOW_CLOSE = 0x7F9B   -- TAM 으로 되돌린 직후
...
emu.log("PROBE 뱅크창 0.1.1 -- 뱅크 교체 중 인터럽트 진입만 센다")
