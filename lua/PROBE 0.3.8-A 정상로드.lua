...
-- 설계 실수였다.
--
-- 이 파일에는 스위치가 없다.  **기록되는 것은 전부 정상 경로다.**
-- 짝은 `PROBE 0.3.8-B 저장후재로드.lua` 이고, 둘은 파일 이름과 출력 경로만 다르다.
--
-- 무엇을 재는가
-- -------------
...
-- 쓰는 법
-- -------
--   1) 디스크를 올리고 **파워사이클** 상태에서 Run
--   2) 게임 내 로드 -> 대사 몇 줄  (화면이 멀쩡한 것을 확인)
--   3) **Stop**
--
...
local OUT = string.format("C:\\snatcher\\dump\\probe_v038a_%s_%s.tsv",
                          BUILD, os.date("%H%M%S"))
...
emu.addMemoryCallback(function()
  ...
  if seen[ptr] <= 3 then
    ...
    emu.log(string.format("[A] HIT src=$%04X(%s) -> dst=$%04X(%s) len=%s f%d",
...
emu.addEventCallback(function()
  ...
  closed = true
  file:close()
  emu.log(string.format("PROBE 0.3.8-A [%s]: 전송 %d · 패치구간 %d -> %s",
    BUILD, moves, hits, OUT))
end, emu.eventType.scriptEnded)

emu.log(string.format("PROBE 0.3.8-A loaded  (빌드 %s)  -- 대조군: 정상 로드", BUILD))
emu.log("  디스크: " .. tostring(ROM))
emu.log("  파워사이클 상태에서 게임 내 로드 -> 대사 몇 줄 -> Stop")
