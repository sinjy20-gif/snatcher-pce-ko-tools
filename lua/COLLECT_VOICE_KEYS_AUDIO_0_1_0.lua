...
-- ADPCM: logs the native 6-byte subtitle key and dumps the complete RAM span.

local MEM=emu.memType.pceMemory
local ADPCM=emu.memType.pceAdpcmRam
local OUT="C:/snatcher/snatcher_tool/logs/voice_key_events_raw_v010.tsv"
local CLIPS="C:/snatcher/snatcher_tool/logs/voice_clips_v010_fresh/"
...
local HEADER="event_type\tevent_id\tsequence\tframe\taudio_type\tkey_text\tkey_hex\tread_address\twrite_address\taudio_length\tplayback_rate\tsector\tclip_file\tduration_frames\toccurrence\tstatus\n"
...
local function append(kind,item,frame,status)
  ...
  f:write(string.format("%s\t%s\t%d\t%d\t%s\t%s\t%s\t%04X\t%04X\t%04X\t%02X\t%06X\t%s\t%d\t%d\t%s\n",
    kind,item.id,item.sequence,frame,item.audioType,item.keyText,item.keyHex,
    item.readAddress or 0,item.writeAddress or 0,item.length or 0,item.rate or 0,
...
local function dumpClip(readAddress,length)
  ...
  local hash=fingerprint(data)
  local name=string.format("v%08X.bin",hash)
  local f=io.open(CLIPS..name,"rb")
  if f then
    local size=f:seek("end") or 0; f:close()
    if size==#data then return name,"existing" end
  end
  f=io.open(CLIPS..name,"wb")
  if not f then return "","dump_failed" end
  f:write(data); f:close(); return name,"saved"
end
local function adpcmKey(sector,writeAddress,rate)
  local text=string.format("ADPCM_%06X_%04X_%02X",sector,writeAddress,rate)
  local hex=string.format("%02X %02X %02X %02X %02X %02X",
    sector&0xFF,(sector>>8)&0xFF,(sector>>16)&0xFF,
    writeAddress&0xFF,(writeAddress>>8)&0xFF,rate&0xFF)
  return text,hex
end
...
local function startAdpcm(s,frame)
  ...
  local keyText,keyHex=adpcmKey(sector,writeAddress,rate)
  ...
  local clip,dumpStatus=dumpClip(readAddress,length)
  active={id=string.format("%s_%04d",session,seq),sequence=seq,audioType="ADPCM",
    ...
    length=length,rate=rate,sector=sector,clip=clip,startFrame=frame,
...
emu.log("COLLECT_VOICE_KEYS_AUDIO 0.1.0 loaded -- 음성 전용")
emu.log("  ADPCM 6-byte key + full RAM clip · CDDA sector range")
