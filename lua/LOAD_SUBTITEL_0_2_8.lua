-- SUBTITEL 0.2.8 로더 -- 엔진 페이로드 + 매직값을 아케이드 카드에 얹는다
--
...
--
-- 0.2.8 케이브는 설치 직전에 AC $1C04F0 의 매직 'K' 'O' 를 확인한다.
...
--
-- 쓰는 법
--   1. 펌웨어를 0.2.8.pce 로 바꾸고 게임을 **다시 로드**한다
...
local PATH = "C:/snatcher/SUBTITEL/0.2.8.bin"
local RETRY = 300

local f = io.open(PATH, "rb")
if f == nil then emu.log("★ 페이로드를 못 열었다: " .. PATH) return end
local data = f:read("a"); f:close()
emu.log(string.format("SUBTITEL 0.2.8 로더 -- 페이로드 %d B", #data))
