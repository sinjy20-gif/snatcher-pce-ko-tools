-- PROBE_SUB_PACK 0.2.0  --  자막 + **음성 수집**을 한 번에
--
-- 왜 합쳤나
-- ---------
-- 0.1.x 는 자막만 했다.  그래서 2026-08-23 에 엔딩까지 달린 판에서 **음성이 하나도
-- 안 남았다.**  덤프 프로브(PROBE_ADPCM_RAM_0.1.1)를 따로 켰어야 했는데, 자막
-- 프로브에 그 기능이 없었다.
--
--     2026-08-20   247 개
--     2026-08-21     1 개
--     2026-08-23     0 개   <- 엔딩까지 갔는데 0
--
-- 플레이 한 번에 둘 다 되게 한다.  따로 켜는 것을 잊을 수 있는 구조를 없앤다.
--
--     자막   팩에 지문이 있는 대사에 자막을 띄운다
--     수집   재생되는 **모든** 음성을 ADPCM RAM 에서 떠서 내용 해시로 저장한다
--
-- 이름이 내용 해시라 **중복이 안 쌓인다.**  이미 있는 247 개는 건너뛰고 새 소리만
-- 늘어난다.  재생 순서(sequence)로 짓지 않는 이유는 세션마다 1 부터 다시 세서
-- 서로 다른 대사가 같은 번호를 받기 때문이다 (2026-08-20: 173 개 중 23 개 겹침).
--
-- 뽑은 .bin 을 소리로 바꾸려면:  python tools/ram_adpcm_to_wav.py
--
-- 0.1.8 에서 온 것: AC 자리표 ($1F1400) · 자리(위/중간/아래) · 19 자 · 끝 주소 열쇠
--
-- 왜 바꿨나
-- ---------
...
-- 출력  로그 + C:/snatcher/dump/probe_sub_pack_0_2_0_<날짜>.tsv
...
local CLIP_DIR = 'C:/snatcher/snatcher_tool/logs/voice_clips/'
local HARVEST = string.format('C:/snatcher/dump/voice_harvest_%s.tsv', os.date('%Y%m%d_%H%M%S'))
local NL = string.char(10)      -- 이스케이프를 안 쓴다 (한 번 깨졌다)
local harvest_file = io.open(HARVEST, 'w')
if harvest_file then
  harvest_file:write('event_id	start_frame	audio_type	read_address	audio_length'
                     .. '	playback_rate	detected_duration	clip_file	in_pack' .. NL)
end
local session = os.date('%Y%m%d_%H%M%S')
local harvested, already, harvest_fail = 0, 0, 0
local pending = nil                       -- 재생 중인 것의 정보

-- 0.1.1 의 것을 그대로 쓴다.  다르면 같은 소리가 다른 이름을 받아 중복이 쌓인다
...
local function grab_clip(read_addr, len)
  if ADPCM_RAM == nil or len <= 0 then return nil end
  local buf, chunk = {}, {}
  for i = 0, len - 1 do
    chunk[#chunk + 1] = string.char(emu.read((read_addr + i) % ADPCM_RAM_SIZE, ADPCM_RAM) or 0)
  ...
  local exists = io.open(CLIP_DIR .. name, 'rb')
  if exists then exists:close(); already = already + 1; return name end

  local out = io.open(CLIP_DIR .. name, 'wb')
  if out == nil then
    harvest_fail = harvest_fail + 1
    if harvest_fail == 1 then
      say('★ 클립을 못 쓴다 -- 폴더가 있나: ' .. CLIP_DIR)
    end
    return nil
  end
...
local function flush()
  local f = io.open(string.format('C:/snatcher/dump/probe_sub_pack_0_2_0_%s.tsv',
...
emu.addEventCallback(function()
  ...
  if playing and not was_playing then
    ...
    local ending = ((read_addr + length) % 0x10000)

    -- ★ 자막이 있든 없든 **항상** 뜬다.  효과음도 나중에 쓸 수 있다
    local clip = grab_clip(read_addr, length)
    pending = { frame = frames, read_addr = read_addr, length = length,
                rate = rate, clip = clip or '' }

    local found = find_parts(ending, rate)
    if #found == 0 then
      ...
      say(string.format('[%d] 끝 %04X  조각 %d개  y=%d  "%s"',
            frames, ending, #found, record.y, record.text))
    end
  ...
  elseif not playing and was_playing then
    ...
    if pending and harvest_file then
      harvest_file:write(string.format(
        '%s_%04d	%d	ADPCM	%04X	%04X	%02X	%.3f	%s	%s' .. NL,
        session, harvested + already, pending.frame, pending.read_addr,
        pending.length, pending.rate, (frames - pending.frame) / 60,
        pending.clip, parts and 'yes' or 'no'))
      harvest_file:flush()
    end
    pending = nil
    parts = nil
  ...
  if frames % 3600 == 0 then
    say(string.format('--- %d --- 띄움 %d · 넘김 %d · 수집 새것 %d · 이미있음 %d',
                      frames, shown, skipped, harvested, already))
    flush()
  end
end, emu.eventType.startFrame)

emu.log('PROBE_SUB_PACK 0.2.0 loaded  --  자막 + 음성 수집')
emu.log('  수집: ' .. CLIP_DIR .. '  (내용 해시 이름 -- 중복 안 쌓인다)')
emu.log('  기록: ' .. HARVEST)
