-- PROBE_GFX_UPLOAD 0.1.4 -- 소스 데이터를 통째로 뜬다 · 가볍게 (2026-08-25)
--
...
local BASE  = "C:\\snatcher\\dump\\gfx_upload_0_1_4_" .. stamp
...
emu.addEventCallback(function()
  ...
  if not dumped and fills > 1000 and lastFill > 0 and frames - lastFill > 20 then
    dumped = true
    dump_all("settled")
    emu.log("GFX 0.1.4 -- 채우기가 멎어 소스를 떴다")
  end

  if frames % 1800 == 0 then
    emu.log(string.format("GFX 0.1.4 -- 버퍼채움 %d · 소스읽기 %d · 떴나 %s",
...
emu.addEventCallback(function()
  ...
  file:write("PROBE_GFX_UPLOAD 0.1.4  " .. stamp .. "\n")
  ...
  file:close()
  emu.log("GFX 0.1.4 보고서 -> " .. BASE .. "_report.txt")
end, emu.eventType.scriptEnded)

emu.log("PROBE_GFX_UPLOAD 0.1.4 -- CD RAM 창($8000-$DFFF)을 통째로 뜬다")
