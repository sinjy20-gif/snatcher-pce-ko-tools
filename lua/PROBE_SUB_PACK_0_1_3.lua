-- PROBE_SUB_PACK 0.1.3  --  0.1.2 + **넘긴 지문을 찍는다**
--
...
-- 출력  로그 + C:/snatcher/dump/probe_sub_pack_0_1_3_<날짜>.tsv
...
local function flush()
  local f = io.open(string.format('C:/snatcher/dump/probe_sub_pack_0_1_3_%s.tsv',
...
if ready then
  ...
  for i = 0, P.adpcm_count - 1 do
    local at = P.adpcm_off + i * P.adpcm_stride
    local key = string.format('%04X/%04X/%02X', u16(at), u16(at + 2), u8(at + 4))
...
emu.addEventCallback(function()
  ...
  if playing and not was_playing then
    ...
    if #found == 0 then
      ...
      local key = string.format('%04X/%04X/%02X',
...
emu.log('PROBE_SUB_PACK 0.1.3 loaded')
