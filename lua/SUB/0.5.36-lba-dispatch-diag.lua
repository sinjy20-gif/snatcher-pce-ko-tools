-- SUB 0.5.36 -- 디스패처가 **실제로 무엇을 읽었는가** (1차 실패 진단)
--
-- 무엇을 시험하나
-- ---------------------------------------------------------------------------
...
if not fh then
  emu.log('SUB 0.5.36 ★★ 표가 없다: ' .. TABLE)
...
if #data % STRIDE ~= 0 then
  emu.log(string.format('SUB 0.5.36 ★★ 표 크기가 %d B -- %d 의 배수가 아니다', #data, STRIDE))
  return
end
for i = 1, #data do
...
emu.log(string.format('SUB 0.5.36 표 %d 항목(%d B)을 AC $%06X 에 올렸다 · 슬롯 $%06X 초기화',
...
do
  ...
  if bad > 0 then
    emu.log(string.format('SUB 0.5.36 ★★ AC 되읽기 불일치 %d/64 -- 표가 그 자리에 없다', bad))
  else
    local e = {}
    for i = 1, 9 do e[#e + 1] = string.format('%02X', data:byte(i)) end
    emu.log('SUB 0.5.36 AC 되읽기 OK · 첫 항목 ' .. table.concat(e, ' '))
  end
end
...
emu.addEventCallback(function()
  ...
  elseif status == 0xA0 then
    ...
    local why
    if r0 == r1 and r1 == r2 then
      why = string.format('읽은값 %02X %02X %02X 가 모두 같다 -> AC 자동증가 안 먹음', r0, r1, r2)
    elseif byLba[readLba] then
      why = string.format('읽은값 %06X 는 표에 있는 LBA -> 주소는 맞다.  비교/분기 논리 문제', readLba)
    else
      why = string.format('읽은값 %06X 가 표에 없다 -> 색인 주소가 틀렸다', readLba)
    end
  ...
  emu.log(string.format('SUB 0.5.36 #%d  st $%02X  lba %06X  native %s  true %s  %d단계  %s',
  ...
  if voices % 10 == 0 then
    emu.log(string.format('SUB 0.5.36 === %d 음성 · OK %d · 오답 %d · miss %d · 훅없음 %d',
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    'SUB 0.5.36 끝 -- 음성 %d · OK %d · 오답 %d · miss(정상) %d · 훅없음 %d',
    voices, okN, badN, missN, silentN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log('SUB 0.5.36-lba-dispatch-check armed')
