-- SUB 0.5.20 -- RCR 을 쓰는 코드가 누구인가.  SEI 창이 닫히는 순간과 붙는가 (쓰기 0 B)
--
-- 0.5.19 가 찾은 것 (§7-F)
-- ---------------------------------------------------------------------------
--     분할선   화면줄 7 · 135 · 148
--     게이트   줄 21~26
--     ★ RCR=135 의 **예약 시점**이 줄 32 에서 199 까지 흔들린다
--       그리고 135 를 지난 뒤 예약된 프레임은 148 예약이 통째로 빠진다
--
-- 안 맞는 것
-- ---------------------------------------------------------------------------
-- 게이트가 줄 26 에 끝나면 줄 135 까지 109 줄이 남는다.  렌더 15 줄로는
-- 143 까지 밀릴 수 없다.  즉 RCR=135 를 쓰는 놈이 게이트 직후에 도는 게 아니라
-- **다른 무언가에 연쇄로 물려 있다.**  그 연쇄를 봐야 한다.
--
-- 무엇을 재나
-- ---------------------------------------------------------------------------
...
local OUT   = 'C:/snatcher/dump/rcr_writer_0_5_20_' .. STAMP .. '.tsv'
local CODE  = 'C:/snatcher/dump/rcr_writer_code_0_5_20_' .. STAMP .. '.txt'
...
-- ---------------------------------------------------------------------------
local FOREIGN = {
  { at = 0x7F4A, bad = NOP,  name = '0.5.16 SEI off' },
...
local function checkForeign()
  ...
  for _, f in ipairs(FOREIGN) do
    if (emu.read(f.at, MEM) or -1) == f.bad then
      dirty = true
      emu.log(string.format('SUB 0.5.20 ★★ 오염 감지 -- $%04X 가 NOP (%s 잔재)', f.at, f.name))
...
local function dumpAround(pc)
  ...
  code:flush()
  emu.log(string.format('SUB 0.5.20 ★ CODE DUMP writer $%04X (%d B)', pc, hi - lo + 1))
end
...
emu.addEventCallback(function()
  ...
  if #list > 0 then
    ...
    if seen[sig] <= 3 or (wo >= 0 and wi >= 0 and (wo - wi) > 60) then
      reported = reported + 1
      emu.log(string.format('SUB 0.5.20 %df · 창[%d..%d] %s · %s',
        frame, wi, wo,
  ...
  emu.drawString(4, 84, string.format(
    '0.5.20 RCR lo/hi %d/%d · writer %d종 · 보고 %d%s',
...
emu.log('SUB 0.5.20-rcr-writer-pc armed -- RCR 을 쓰는 PC 와 SEI 창의 관계를 잰다')
