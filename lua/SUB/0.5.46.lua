-- SUB 0.5.46 -- 전체 ADPCM 마스터 native LBA 관찰
--
...
local OUT = 'C:/snatcher/dump/sub/adpcm_native_046.tsv'

local fh = io.open(TABLE, 'rb')
if not fh then
  emu.log('SUB 0.5.46 표가 없다: ' .. TABLE)
  return
end
local data = fh:read('*a')
fh:close()
if #data == 0 or #data % STRIDE ~= 0 then
  emu.log(string.format('SUB 0.5.46 표 크기 오류: %d B', #data))
  return
end
...
emu.addEventCallback(function()
  ...
  if not (playing and not prev) then prev = playing; return end
  prev = playing
  voices = voices + 1
  ...
  emu.log(string.format('SUB 0.5.46 #%d ALL st $%02X lba %06X build $%02X %s',
    voices, status, lba, t[12], verdict))
  for i = 0, 12 do emu.write(AC_SLOT + i, 0, AC) end
end, emu.eventType.endFrame)

emu.addEventCallback(function()
  if out then out:close() end
  emu.log(string.format('SUB 0.5.46 끝 -- 전체 %d · OK %d · 오답 %d · miss %d · 훅없음 %d',
    voices, okN, badN, missN, noHookN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log(string.format('SUB 0.5.46 loaded -- 전체 ADPCM %d 항목을 AC $%06X 에 업로드', entries, AC_INDEX))
