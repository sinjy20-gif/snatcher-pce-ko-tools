-- SUB 0.3.9 -- boot AC translation + subtitle verifier (read-only)
-- 0.4.5.9-boot-ac-subtitle CUE/BIOS 전용. 이 파일 하나만 실행한다.
...
local translation = readFile('C:/snatcher/build/patch/0.4.5.9-collection-base/ac_dynamic_packs/disc_package_image.bin')
-- Boot loader publishes the same state that the old first-demand init_ok did.
...
-- Old translation helper reaches this only when AC magic is absent.
emu.addMemoryCallback(function()
  demandInit = true
  emu.log('SUB 0.3.9 ★ FAIL: 첫 한글에서 기존 수요 적재 init_store가 실행됨')
end, emu.callbackType.exec, 0xBD13, 0xBD13, emu.cpuType.pce, CPU)

emu.addMemoryCallback(function()
  if checked then return end
  checked = true
  emu.log('SUB 0.3.9 first Korean lookup -- AC byte audit start')
  local rows = {
  ...
  if total == 0 and not demandInit then
    emu.log('SUB 0.3.9 ★ BOOT AC PASS: 번역 전체 + 자막 3종 byte exact · 수요 적재 0')
  else
    emu.log(string.format('SUB 0.3.9 ★ BOOT AC FAIL: mismatch=%d demand_init=%s',
                          total, tostring(demandInit)))
  end
end, emu.callbackType.exec, 0x5E40, 0x5E40, emu.cpuType.pce, CPU)

emu.log('SUB 0.3.9 loaded -- boot AC verifier / 완전 읽기 전용')
emu.log('  새 CUE+BIOS로 전원 재시작 후 코나미 빌딩 또는 접수처 첫 한글까지 진행')
