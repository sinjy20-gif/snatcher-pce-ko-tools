-- 국장실 소환 -- **우리 arm 인가 게임 부하인가**.  0.5.174
--
-- 0.5.174 이 답한 것 / 못 답한 것
-- -------------------------------
-- 답했다:
...
--
-- ★ 0.5.174 의 동작부는 **글자 그대로** 둔다.  그 판은 돌아가는 것이 확인됐다.
...
--
-- 산출물  C:/snatcher/dump/byr_gate_0_5_174_<시각>.tsv
...
local GATE = 0xFEC4              -- 우리 게이트 (자막 arm 진입점)
local OUR_BANK = 0x01            -- ⚠ MPR7==$01 일 때만 우리 코드다 ($00 은 원본 BIOS)

local OUT = 'C:/snatcher/dump/byr_gate_0_5_174_' .. os.date('%Y%m%d_%H%M%S') .. '.tsv'
...
local function report(e, why)
  ...
  emu.log(string.format(
    '0.5.174 [ep%d %s] 프레임 %d~%d (%d) · 표본 %d · ★늦음 %d '
...
emu.addMemoryCallback(function()
  ...
  if type(m7) ~= 'number' then return end
  -- ⚠ 같은 주소에 원본 BIOS 코드도 있다.  뱅크로 갈라야 한다
  if math.floor(m7) ~= OUR_BANK then c_gate_other = c_gate_other + 1 return end
  c_gate = c_gate + 1
  local at = scanline()
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    '0.5.174 %df · port3 %d · reg8 %d · byr96 %d · 기록 %d · 게이트(우리) %d · (원본뱅크) %d',
    frame, c_port3, c_reg8, c_byr96, c_written, c_gate, c_gate_other))
  if c_port3 == 0 then
...
emu.log('SUB 0.5.174-byr-summon-pc  ★ 읽기 전용 · 아무것도 안 깐다')
