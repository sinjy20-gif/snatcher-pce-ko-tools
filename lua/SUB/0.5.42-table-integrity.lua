-- SUB 0.5.42 -- 표가 **읽는 그 순간까지 살아 있는가** (0.5.40 + 무결성 확인)
--
...
local OUT = 'C:/snatcher/dump/table_integrity_0_5_42_' .. STAMP .. '.tsv'
...
if not fh then
  emu.log('SUB 0.5.42 ★★ 표가 없다: ' .. TABLE)
...
if #data % STRIDE ~= 0 then
  emu.log(string.format('SUB 0.5.42 ★★ 표 크기가 %d B -- %d 의 배수가 아니다', #data, STRIDE))
  return
end
for i = 1, #data do
...
emu.log(string.format('SUB 0.5.42 표 %d 항목(%d B)을 AC $%06X 에 올렸다 · 슬롯 $%06X 초기화',
...
do
  ...
  if bad > 0 then
    emu.log(string.format('SUB 0.5.42 ★★ AC 되읽기 불일치 %d/64 -- 표가 그 자리에 없다', bad))
  else
    local e = {}
    for i = 1, 9 do e[#e + 1] = string.format('%02X', data:byte(i)) end
    emu.log('SUB 0.5.42 AC 되읽기 OK · 첫 항목 ' .. table.concat(e, ' '))
  end
end

-- ---------------------------------------------------------------------------
-- ★ 0.5.42 가 더한 것 -- 표가 **읽는 그 순간까지 살아 있는가**
-- ---------------------------------------------------------------------------
...
emu.addEventCallback(function()
  ...
  if bad > 0 and not brokeAt then
    ...
    emu.log(string.format(
      'SUB 0.5.42 ★★ 표가 깨졌다 -- %d 프레임 근처 · 앞 %d B 중 %d B 불일치',
    ...
    emu.log('   => 포트가 아니라 **자리** 문제다.  $1F2400 은 한가하지 않다')
  elseif bad == 0 and brokeAt and not brokeShown then
    brokeShown = true
    emu.log('SUB 0.5.42 (표가 다시 맞다 -- 덮은 쪽이 되돌렸거나 일시적이었다)')
  end
...
emu.addEventCallback(function()
  ...
  emu.log(string.format('SUB 0.5.42 #%d  BUILD $%02X (기대 $74)  st $%02X  lba %06X  %d단계',
    voices, slot[12], status, lba, slot[11]))
  emu.log('            ' .. verdict)

  -- ★ 이 음성을 읽던 그 순간에 표가 살아 있었는가.  이것이 0.5.42 의 요점이다.
  do
    local bad = tableBad()
    if bad == 0 then
  ...
  if voices % 10 == 0 then
    emu.log(string.format('SUB 0.5.42 === %d 음성 · OK %d · 오답 %d · miss %d · 훅없음 %d',
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    'SUB 0.5.42 끝 -- 음성 %d · OK %d · 오답 %d · miss(정상) %d · 훅없음 %d',
    voices, okN, badN, missN, silentN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log('SUB 0.5.42-table-integrity armed')
emu.log('  ★ BIOS 는 0.4.7.4-lba-probe (BUILD $74) · CUE 는 0.4.6.22 것 그대로')
