-- SUB 0.5.35 -- 네이티브 LBA 디스패처가 실기에서 옳은 답을 내는가
--
-- 무엇을 시험하나
-- ---------------------------------------------------------------------------
...
local OUT = 'C:/snatcher/dump/lba_dispatch_0_5_35_' .. STAMP .. '.tsv'
...
if not fh then
  emu.log('SUB 0.5.35 ★★ 표가 없다: ' .. TABLE)
...
if #data % STRIDE ~= 0 then
  emu.log(string.format('SUB 0.5.35 ★★ 표 크기가 %d B -- %d 의 배수가 아니다', #data, STRIDE))
  return
end
for i = 1, #data do
...
emu.log(string.format('SUB 0.5.35 표 %d 항목(%d B)을 AC $%06X 에 올렸다 · 슬롯 $%06X 초기화',
...
emu.addEventCallback(function()
  ...
  elseif status == 0xA0 then
    ...
    if inTable then badN = badN + 1; verdict = 'MISS-BUT-IN-TABLE'
    else missN = missN + 1; verdict = 'miss(정상 · 표에 없음)' end
  ...
  emu.log(string.format('SUB 0.5.35 #%d  st $%02X  lba %06X  native %s  true %s  %d단계  %s',
  ...
  if voices % 10 == 0 then
    emu.log(string.format('SUB 0.5.35 === %d 음성 · OK %d · 오답 %d · miss %d · 훅없음 %d',
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    'SUB 0.5.35 끝 -- 음성 %d · OK %d · 오답 %d · miss(정상) %d · 훅없음 %d',
    voices, okN, badN, missN, silentN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log('SUB 0.5.35-lba-dispatch-check armed')
