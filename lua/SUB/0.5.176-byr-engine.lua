-- 국장실 소환 -- **엔진 복사가 돈 프레임인가**.  0.5.176
--
-- ★★ 0.5.176 가 못 가른 것 -- 게이트는 **매 프레임** 돈다
...
--
-- ---- 아래는 0.5.176 머리말 ----
-- 국장실 소환 -- **우리 arm 인가 게임 부하인가**.  0.5.176
--
-- ★★ 0.5.176 를 버린 이유 -- 뱅크 검사가 거꾸로였다
-- ---------------------------------------------------
-- 0.5.176 는 `MPR7 == $01` 일 때만 우리 게이트로 셌다.  결과가 **게이트(우리) 0 ·
...
--
-- ★ 0.5.176 의 동작부는 **글자 그대로** 둔다.  그 판은 돌아가는 것이 확인됐다.
...
--
-- 산출물  C:/snatcher/dump/byr_gate_0_5_176_<시각>.tsv
...
local OUT = 'C:/snatcher/dump/byr_gate_0_5_176_' .. os.date('%Y%m%d_%H%M%S') .. '.tsv'
local out = io.open(OUT, 'w')
if out then out:write('frame\tep\twrite_line\tpc\tmpr7\tmpr5\tband\tgate\teng\n') end
...
local function report(e, why)
  ...
  emu.log(string.format(
    '0.5.176 [ep%d %s] 프레임 %d~%d (%d) · 표본 %d · ★늦음 %d '
...
emu.addMemoryCallback(function(address, value)
  ...
  if band ~= 'ok' then
    ...
    local g = (gateLine >= 0 and ('g' .. gateLine) or 'g-') .. '/e' .. engWrites
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    '0.5.176 %df · port3 %d · reg8 %d · byr96 %d · 기록 %d · 게이트 %d · $1A10 쓰기 %d',
...
emu.log('SUB 0.5.176-byr-summon-pc  ★ 읽기 전용 · 아무것도 안 깐다')
