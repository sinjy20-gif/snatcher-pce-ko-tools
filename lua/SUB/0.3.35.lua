...
local VERSION = rawget(_G, 'SUB_ALLOCATOR_VERSION') or '0.3.35'
...
local function reportShortfall(used)
  ...
  if aBest >= N then
    emu.log('  -> 전체 VRAM 에는 자리가 있다.  창 확장(패턴 상위 비트)으로 해결 가능')
  else
    emu.log('  -> VRAM 어디에도 연속 자리가 없다.  연속성을 깨거나 N 을 줄여야 한다')
  end
end
