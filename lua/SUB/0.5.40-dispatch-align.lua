-- SUB 0.5.40 -- 디스패처가 **실제로 무엇을 읽었는가** (1차 실패 진단)
--
-- 무엇을 시험하나
-- ---------------------------------------------------------------------------
...
local OUT = 'C:/snatcher/dump/dispatch_align_0_5_40_' .. STAMP .. '.tsv'
...
if not fh then
  emu.log('SUB 0.5.40 ★★ 표가 없다: ' .. TABLE)
...
if #data % STRIDE ~= 0 then
  emu.log(string.format('SUB 0.5.40 ★★ 표 크기가 %d B -- %d 의 배수가 아니다', #data, STRIDE))
  return
end
for i = 1, #data do
...
emu.log(string.format('SUB 0.5.40 표 %d 항목(%d B)을 AC $%06X 에 올렸다 · 슬롯 $%06X 초기화',
...
do
  ...
  if bad > 0 then
    emu.log(string.format('SUB 0.5.40 ★★ AC 되읽기 불일치 %d/64 -- 표가 그 자리에 없다', bad))
  else
    local e = {}
    for i = 1, 9 do e[#e + 1] = string.format('%02X', data:byte(i)) end
    emu.log('SUB 0.5.40 AC 되읽기 OK · 첫 항목 ' .. table.concat(e, ' '))
  end
end
...
emu.addEventCallback(function()
  ...
  emu.log(string.format('SUB 0.5.40 #%d  BUILD $%02X (기대 $74)  st $%02X  lba %06X  %d단계',
  ...
  if voices % 10 == 0 then
    emu.log(string.format('SUB 0.5.40 === %d 음성 · OK %d · 오답 %d · miss %d · 훅없음 %d',
...
emu.addEventCallback(function()
  ...
  emu.log(string.format(
    'SUB 0.5.40 끝 -- 음성 %d · OK %d · 오답 %d · miss(정상) %d · 훅없음 %d',
    voices, okN, badN, missN, silentN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log('SUB 0.5.40-lba-dispatch-check armed')
