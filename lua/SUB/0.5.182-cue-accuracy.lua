...
--
-- 산출물  C:/snatcher/dump/cue_accuracy_0_5_182_<시각>.tsv

local MEM, CPU = emu.memType.pceMemory, emu.cpuType.pce

local STAMP = os.date('%Y%m%d_%H%M%S')
local PATH  = 'C:/snatcher/dump/cue_accuracy_0_5_182_' .. STAMP .. '.tsv'
...
-- EXPECT[1] 은 무장 때 이미 심겨 있어 '변화' 로 안 잡힌다 (위 주석 참고).
...
emu.addEventCallback(function()
  ...
  -- ★ 0.5.181 이 여기서 틀렸다.  렌더러는 **1 번 구간의 record_ptr 을 이미 심은
  --   채로 시작한다** (`record_ptr_initial` 이 렌더러에 구워져 있다.
  --   build_subtitle_engine_cdda_scheduled.py 의 first_ptr).  그래서 프로브가
  --   보는 첫 '변화' 는 이미 **2 번 구간**이다.  seq 를 그대로 색인에 쓰면
  --   전 구간이 한 칸씩 밀려 "3 초 밀린다" 는 거짓 결과가 나온다 -- 실제로
  --   0.5.5 는 오차 1 프레임짜리 정상판이었다 (소유자가 눈으로 알아챘다).
  local exp = (track == 0x17) and EXPECT[seq + 1] or nil
...
say('SUB 0.5.182-cue-accuracy armed -- 순수 관측 (★0.5.5 에 올릴 것)')
say('  ⚠ 정상 속도로 · 트랙 17 오프닝을 **끝까지** 틀 것 (35 구간 중 34 개가 잡힌다)')
say('  ' .. PATH)
