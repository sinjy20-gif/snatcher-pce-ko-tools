-- SUB 0.4.23 -- portrait 1/2/3 SATB position capture / read-only.
--
-- Stable portrait screen, game window focused:
--   Q = one portrait, W = two portraits, E = three portraits
...
local ROOT = 'C:/snatcher/dump/sub_0_4_23_portrait_compare_' .. stamp
...
local held = { Q = false, W = false, E = false }
...
local function capture(caseCount)
  ...
  for slot = 0, 63 do
    ...
    slotsFile:write(string.format(
      '%d\t%d\t%d\t%02X\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%04X\t%04X\t%04X\t%d\t%d\t%d\t%d\t%s\n',
      caseCount, captureSeq, frame, state, s.slot, s.active and 1 or 0,
  ...
  for i, g in ipairs(groups) do
    groupsFile:write(string.format(
      ...
      caseCount, captureSeq, frame, i, g.x0, g.y0, g.x1, g.y1,
      g.x1 - g.x0 + 1, g.y1 - g.y0 + 1, #g.slots,
      table.concat(g.slots, ','), table.concat(g.patterns, ',')))
  end
  slotsFile:flush(); groupsFile:flush()
  emu.log(string.format(
    'SUB 0.4.23 ★ CASE %d CAPTURED: active=%d visible=%d subtitle_hint=%d game_groups=%d f=%d',
    caseCount, activeCount, visibleCount, subtitleCount, #groups, frame))
end

emu.addEventCallback(function()
  frame = frame + 1
  local map = { Q = 1, W = 2, E = 3 }
  for key, caseCount in pairs(map) do
    local down = keyDown(key)
    if down and not held[key] then capture(caseCount) end
    held[key] = down
  end
...
emu.addEventCallback(function()
  ...
  emu.log('SUB 0.4.23 saved: ' .. SLOTS_OUT)
  emu.log('SUB 0.4.23 saved: ' .. GROUPS_OUT)
end, emu.eventType.scriptEnded)

emu.log('SUB 0.4.23 loaded -- portrait 1/2/3 SATB comparison / read-only')
emu.log('  game window focus: Q=1 portrait · W=2 portraits · E=3 portraits')
