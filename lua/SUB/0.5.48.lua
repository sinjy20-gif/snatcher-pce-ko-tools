-- SUB 0.5.48 -- 전체 ADPCM LBA 관찰 + 경로 카운터
--
...
local OUT = 'C:/snatcher/dump/sub/adpcm_native_048.tsv'
...
if out then
  out:write('voice\tframe\tstatus\tlba\tnative_key\ttrue_key\tprobes\tbuild\thook\tad_trans\tissue\tcd_read\tverdict\n')
end
...
local hookN, transN, issueN, cdReadN = 0, 0, 0, 0
...
emu.addEventCallback(function()
  ...
  if playing and not prev then
    ...
    pendingCounts = { hookN, transN, issueN, cdReadN }
    prev = true
    return
  end
  prev = playing
  ...
  pendingState = nil
  local pc = pendingCounts or { hookN, transN, issueN, cdReadN }
  pendingCounts = nil
  voices = voices + 1
  ...
  if out then
    out:write(string.format('%d\t%d\t%02X\t%06X\t%s\t%s\t%d\t%02X\t%d\t%d\t%d\t%d\t%s\n',
      voices, num(s, 'ppu.frameCount'), status, lba, hex6(native), hex6(real),
      t[11], t[12], hookN - pc[1], transN - pc[2], issueN - pc[3], cdReadN - pc[4], verdict))
    out:flush()
  end
  emu.log(string.format('SUB 0.5.48 #%d ALL wait %d st $%02X lba %06X build $%02X path H%d/T%d/I%d/R%d %s',
    voices, pendingAge, status, lba, t[12], hookN - pc[1], transN - pc[2],
    issueN - pc[3], cdReadN - pc[4], verdict))
  for i = 0, 12 do emu.write(AC_SLOT + i, 0, AC) end
end, emu.eventType.endFrame)

emu.addEventCallback(function()
  if out then out:close() end
  emu.log(string.format('SUB 0.5.48 끝 -- 전체 %d · OK %d · 오답 %d · miss %d · 훅없음 %d',
    voices, okN, badN, missN, noHookN))
  emu.log('  ' .. OUT)
end, emu.eventType.scriptEnded)

emu.log(string.format('SUB 0.5.48 loaded -- 전체 ADPCM %d 항목을 AC $%06X 에 업로드', entries, AC_INDEX))
