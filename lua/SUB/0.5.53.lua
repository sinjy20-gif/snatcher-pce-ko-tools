-- SUB 0.5.53 -- 0.4.6.32 native armer 관찰 전용
...
local function snapshot(label, status, lba, state)
  emu.log(string.format(
    'SUB 0.5.53 %s f%d slot $%02X lba %06X state $%02X cpuMagic %s ready $%02X',
...
emu.addEventCallback(function()
  ...
  if status ~= lastStatus or lba ~= lastLba then
    emu.log(string.format('SUB 0.5.53 f%d slot $%02X lba %06X state $%02X',
...
emu.log('SUB 0.5.53 native armer probe loaded -- read only')
emu.log('  BIOS 0.4.6.32 · D000 A1/A2 전후 자동 출력 · 키 입력 불필요')
