-- PROBE_STALL 0.1.0  --  "다음 음원이 안 나온다" 를 잰다
--
-- 무엇을 재나
-- -----------
...
local OUTPATH = 'C:/snatcher/dump/probe_stall_0_1_0_' .. STAMP .. '.tsv'
...
emu.addEventCallback(function()
  say('')
  say(string.format('== PROBE_STALL == %d 프레임 (%.1f 초)', frames, frames / 60.0))
  ...
  say('')
  say('[1] CPU 가 머문 곳 (PC 를 256 B 칸으로)')
  for i = 1, math.min(6, #hot) do
    say(string.format('    $%04X-$%04X   %6d 프레임  %5.1f%%',
      hot[i].b, hot[i].b + 0xFF, hot[i].n, 100.0 * hot[i].n / math.max(1, frames)))
  end
  if #hot > 0 and hot[1].n > frames * 0.5 then
    say('    ★ 절반 넘게 한 동네에 있다 -- 여기가 기다리는 루프다')
  end

  -- 2) ADPCM 포트
  say('')
  say('[2] ADPCM 포트 $1800-$180F')
  local any = false
  for a = PORT_LO, PORT_HI do
    local r, w = port_read[a] or 0, port_write[a] or 0
    if r > 0 or w > 0 then
      any = true
      say(string.format('    $%04X  읽기 %8d · 쓰기 %8d', a, r, w))
    end
  end
  if not any then say('    (한 번도 안 건드렸다)') end

  -- 3) 무엇이 얼어 있나  ★ 이게 핵심이다
  say('')
  say('[3] cdrom 상태 -- 마지막까지 안 변한 것 (기다림의 대상)')
  local cold, warm = {}, {}
  ...
  say(string.format('    한 번도 안 변한 키 %d 개:', #cold))
  for i = 1, math.min(10, #cold) do
    say(string.format('       %-38s = %s', cold[i], prev[cold[i]] or '?'))
  end
  say('    변했다가 멈춘 지 오래된 것 (마지막 변화 이후 프레임):')
  for i = 1, math.min(8, #warm) do
    say(string.format('       %-38s 얼음 %6d f · 변화 %d 회',
...
emu.log('PROBE_STALL 0.1.0 loaded  --  아무것도 안 쓴다 · ADPCM RAM 도 안 읽는다')
emu.log('  멈춘 뒤 10 초쯤 두었다가 Stop 을 눌러야 표가 나온다')
emu.log('  ' .. OUTPATH)
