-- PROBE_SUB_PACK 0.1.10 -- sector + 끝주소 + 재생률로 ADPCM을 식별한다
--
-- 왜 바꿨나
-- ---------
...
if ready then
  ...
  for i = 0, P.adpcm_count - 1 do
    ...
    local key = string.format('sector %06X 끝 %04X 률 %02X', sector, u16(at + 3), u8(at + 5))
    if not listed[key] then
      listed[key] = true
      emu.log(string.format('  팩에 있는 지문  %s  "%s"', key,
                            read_record(u32(at + 9)).text))
    end
  end
end
...
emu.addEventCallback(function()
  ...
  if playing and not was_playing then
    ...
    local found = sector and find_parts(sector, ending, s['cdrom.adpcm.playbackRate'] or 0) or {}
    if #found == 0 then
      skipped = skipped + 1                -- 자막이 없다 = 효과음이거나 아직 번역 전
      local key = string.format('sector %s 끝 %04X 률 %02X  (%04X+%04X)',
            sector and string.format('%06X', sector) or '없음', ending,
      ...
      say(string.format('[%d] sector %06X 끝 %04X  조각 %d개  y=%d  "%s"',
...
emu.log('PROBE_SUB_PACK 0.1.10 loaded')
