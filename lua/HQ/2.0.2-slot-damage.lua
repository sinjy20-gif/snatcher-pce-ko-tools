-- ★ HQ 2.0.2 -- 게임의 쓰기가 우리 **실행 코드**를 실제로 망가뜨리는가
--
-- 왜 이 판인가
-- ------------
-- §39 의 결론은 아직 **추론**이다:
...
emu.addMemoryCallback(function(address, value)
  -- ★ 2.0.1 수정.  2.0.0 은 여기서 `rb(STATE_AT) ~= 0x02` 로 걸렀는데,
  --   그 순간 뱅크가 바뀌어 있으면 $7FDF 가 EA 로 읽힌다 (실측: 37.28 초 줄의
  --   state=EA).  그래서 게임 쓰기가 **전부 걸러져 목록이 비었다.**
  --   대신 "기준 사본을 뜬 뒤" (= 무장 뒤) 인지로 창을 잡는다.
  if ref == nil then return end
  local s = emu.getState() or {}
  local pc = s['cpu.pc'] or 0
  -- ★ 2.0.2 수정.  2.0.1 은 `pc >= 0x7F00` 으로 상주부를 걸렀는데, 그러면
  --   게임 코드 $8xxx·$9xxx 가 **전부 걸러진다** (상주부는 $7F00~$7FFF 뿐).
  --   그래서 목록이 또 비었다.  범위로 정확히 자른다.
  if pc >= SLOT_LO and pc <= SLOT_HI then return end          -- 우리 렌더러/헬퍼
  if pc >= 0x7F00 and pc <= 0x7FFF then return end            -- 상주부
  if pc >= 0xEC00 and pc <= 0xF0FF then return end            -- 스케줄러
  if pc >= 0xF100 then return end                             -- BIOS 뱅크 1
