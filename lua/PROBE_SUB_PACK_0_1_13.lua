-- PROBE_SUB_PACK 0.1.13  --  CD-DA 자막을 띄운다
--
-- 인계서 §2.2 "막힌 것 2" 가 이것이다
-- ---------------------------------------------------------------------------
-- 팩에는 CD-DA 색인이 **이미 구워져 있다** (327 구간).  그런데 Lua 가 CD-DA 를
-- 아예 안 봤다.  ADPCM 만 구현돼 있었다.  그래서 컷신 자막 194 클립이 번역을
-- 다 넣고도 한 줄도 안 떴다 -- 번역이 모자라서가 아니라 읽는 쪽이 없어서다.
--
-- 판정은 ADPCM 보다 쉽다.  구간이 겹치지 않게 구워져 있으므로(검증기 확인),
-- 지금 섹터가 어느 구간에 드는지만 보면 된다:
--
--     lba_from <= cdrom.audioPlayer.currentSector <= lba_to
--
-- 색인이 lba_from 으로 정렬돼 있어 이분 탐색이면 되고, 327 항목이어도 비교
-- 9 번이다.
--
-- ADPCM 과 겹치지 않게
-- ---------------------------------------------------------------------------
-- 음성이 나오는 동안에는 CD-DA 를 아예 안 본다.  둘이 동시에 뜨면 뒤엣것이
-- 앞엣것의 VRAM/RAM 백업을 덮어써서 되돌리기가 깨진다.
--
-- 실측 (PROBE_STALL): CD-DA 는 초당 71~74 섹터로 움직인다 (CD 규격 75).
-- 그러므로 한 구간이 132 섹터면 약 1.8 초다.
--
-- "다음 음성이 안 나온다" 의 원인 (2026-08-24 · PROBE_STALL 0.1.0 으로 잼)
-- ---------------------------------------------------------------------------
-- 음성이 끝난 뒤 게임이 ADPCM 포트를 **초당 68,400 회 읽고 8,400 회 쓰며** 영원히
-- 돈다.  포인터는 얼어 있다:
--
--     playing 0 · readAddr 26624($6800) · writeAddr 26624 · length 0
--
-- 게임이 죽은 것이 아니라 **스크립트가 못 나아가는 것**이다.  왜:
--
--     엔진        $5B80 - $5CE9   362 B
--     VM 스택     $5C40 - $5E1F   스크립트 가상머신 스택
--     겹침        $5C40 - $5CE9   170 B      ★
--
-- 0.1.11 까지의 `disarm()` 은 **매직 3 B 만 지웠다.**  VRAM 은 떠 뒀다 돌려주면서
-- RAM 은 안 돌려줬다.  그래서 엔진 코드 359 B 가 VM 스택 위에 뭉개진 채 남고,
-- 음성이 끝난 뒤부터 스크립트가 자기 스택을 못 믿게 된다.
--
-- 인계서가 이미 적어 둔 것이다 -- "$5C40-$5E1F 상주는 **무효**(스크립트 VM 스택)
-- -> **음성 중에만 빌린다**".  빌린다는 것은 돌려준다는 뜻인데 RAM 만 빠져 있었다.
--
-- 그래서 VRAM 과 똑같이 다룬다: 음성 시작에 떠 두고 끝나면 되돌린다.
--
-- 0.1.10 이 왜 하나도 못 맞췄나 (2026-08-24 실기)
-- ---------------------------------------------------------------------------
-- `audio_length` 가 **수집 때와 재생 때가 다르다.**
--
--     팩 6795 / 런타임 6792     팩 5F89 / 5F83     팩 9F90 / 9F8D
--
-- 읽기주소도 같이 흔들리고 **둘의 합(끝 주소)만 안 변한다.**  0.1.10 은 길이를
-- 열쇠에 넣어서 56 이벤트가 하나도 안 맞았다 -- "팩에 없다" 만 나왔다.
--
-- 그래서 sector(신원) + 끝주소(안 변하는 값) 로 건다.  둘 다 재생 시작 프레임에
-- 그대로 읽힌다.
--
...
local function parse_pack()
  ...
  -- CD-DA 색인.  머리 배치: count u16 @20 · off u32 @22 · stride u8 @43
  P.cdda_count, P.cdda_off = u16(20), u32(22)
  P.cdda_stride = u8(43)
  -- 판이 안 맞으면 조용히 엉뚱한 자막을 띄운다.  여기서 크게 막는다.
  if P.cdda_stride ~= 10 then
    say(string.format('★ CD-DA 항목이 %d B 다 (10 이어야 한다)', P.cdda_stride))
    return false
  end
...
local function find_parts(sector, end_addr)
  local out = {}
  for i = 0, P.adpcm_count - 1 do
    local at = P.adpcm_off + i * P.adpcm_stride
    if u24(at) == sector and u16(at + 3) == end_addr then
...
-- CD-DA 구간 찾기.  색인은 lba_from 으로 정렬돼 있고 구간이 안 겹친다.
-- 항목: lba_from u24 · lba_to u24 · rec_off u24 · flags u8
local function find_cdda(sector)
  local lo, hi = 0, P.cdda_count - 1
  while lo <= hi do
    local mid = (lo + hi) // 2
    local at = P.cdda_off + mid * P.cdda_stride
    local from, to = u24(at), u24(at + 3)
    if sector < from then
      hi = mid - 1
    elseif sector > to then
      lo = mid + 1
    else
      return u24(at + 6), from, to
    end
  end
  return nil
end
...
local cdda_rec = nil                      -- 지금 떠 있는 CD-DA 기록 오프셋
local cdda_shown = 0
...
-- 백스톱이 쓰는 것들을 **여기서 미리 선언**한다.
-- 아래쪽에서 local 로 새로 선언하면 inspect 는 그것을 못 본다 (Lua 스코프).
-- 실제로 그것 때문에 [4]/[5] 가 자리가 정해진 뒤에도 계속 "안 정해졌다" 였다.
local satb, sat_field, is_ours
...
local function inspect(n)
  ...
  -- [4][5] 는 **정해진 SATB 자리**로 읽는다.
  -- 0.1.8 은 `SATB_WORD = 0x1000` 을 워드로 읽었는데 그것이 틀렸다.  실측으로
  -- 자리가 **바이트 $2000** 임이 확정됐다 (2026-08-24 · 우리 엔트리 16 개).
  -- 틀린 자리를 읽던 시절의 값이 "64/64 · 우리 것 0" 이었다 -- 그걸 이상 징후로
  -- 읽고 두 번 헛다리를 짚었다.
  local sat = {}
  if satb == nil then
    say('    [4] SATB 자리가 아직 안 정해졌다 (민 프레임이 있어야 정해진다)')
  else
    for i = 0, 3 do
      sat[#sat+1] = string.format('%04X', sat_field(satb, 0, i))
    end
    say(string.format('    [4] SATB[0] y=%s x=%s pat=%s attr=%s  (%s)',
          sat[1], sat[2], sat[3], sat[4], satb.name))
  end

  local mine, used = 0, 0
  if satb ~= nil then
    for e = 0, 63 do
      if sat_field(satb, e, 0) ~= 0 then used = used + 1 end
      if is_ours(sat_field(satb, e, 2), sat_field(satb, e, 3)) then mine = mine + 1 end
    end
  end
  say(string.format('    [5] 넣으려던 %d 칸 · SATB 에 들어간 우리 것 %d · 쓰인 슬롯 %d/64',
        n, mine, used))
  -- 64/64 는 정상이다.  게임은 안 쓰는 슬롯도 0 으로 채우며 매 프레임 64 칸
  -- 전부를 자기 것으로 다시 쓴다 (1_BREAKTHROUGH §2).
  -- $17 은 $601E 가 $3F 로 세우고 푸시가 깎는 **예산**이다.  조립이 끝난 뒤라
  -- 음수(=255)로 보이는 것이 정상이다 (2_EXTEND §1).  $16 도 조립 뒤 남은 값이다.
  say(string.format('    [6] zp $16 = %d · $17 = %d  (조립 뒤라 255 가 정상)',
        emu.read(0x16, MEM), emu.read(0x17, MEM)))
end

-- 엔진이 앉을 자리를 떠 둔다.  그 자리의 뒷부분 170 B 가 스크립트 VM 스택이라
-- (§머리말) 원래 내용을 돌려주지 않으면 음성이 끝난 뒤 스크립트가 멈춘다.
local ram_backup = nil
local ram_restored = 0

local function ram_save()
  ram_backup = {}
  for i = 0, #engine - 1 do
    ram_backup[i] = emu.read(ENGINE_AT + i, MEM) or 0
  end
end

local function ram_restore()
  if ram_backup == nil then return end
  for i = 0, #engine - 1 do
    emu.write(ENGINE_AT + i, ram_backup[i], MEM)
  end
  ram_backup = nil
  ram_restored = ram_restored + 1
end

local function stage(record)
  vram_save()
  ram_save()                              -- ★ 엔진을 얹기 **전에** 떠 둔다
...
local function disarm()
  -- 매직만 지우면 안 된다.  엔진 코드 359 B 가 VM 스택 위에 남는다.
  ram_restore()                           -- 빌린 RAM 을 돌려준다 (매직도 같이 사라진다)
  armed = false
  vram_restore()                          -- 빌린 VRAM 을 돌려준다
end

local function flush()
  local f = io.open(string.format('C:/snatcher/dump/probe_sub_pack_0_1_12_%s.tsv',
...
-- ==========================================================================
--
-- 0.1.8 에는 없던 증상이 0.1.9 부터 났다: **음성 하나 나오자마자 진행이 멈춘다.**
-- 게임이 얼어붙는 것은 아니다 (미카 눈은 깜빡인다).  스크립트가 음성 끝을
-- 기다리는 모양이다.
--
-- 0.1.9 가 얹은 것은 셋이다.  하나씩 꺼서 가른다.
--
--     MEASURE    재생 시작 프레임에 ADPCM RAM 512 회 읽기 + 표 한 줄 쓰기
--                ★ 제일 의심스럽다.  ADPCM RAM 을 재생 중에 읽는 것이므로
--     BACKSTOP   $5C06 exec 콜백 + SATB 훑기
--     (자막)     원래 있던 것.  이건 안 끈다
--
-- 가르는 법 (한 판에 하나씩)
--     1  MEASURE = false          안 멈추면 -> 지문 읽기가 범인
--     2  BACKSTOP = false         안 멈추면 -> $5C06 콜백이 범인
--     3  둘 다 true 인데 안 멈추면 -> 멈춤이 이 스크립트 탓이 아니다
--
-- ★ 끄고 돌린 판은 열쇠 측정표가 안 나온다.  가른 뒤 다시 켤 것.
--
-- 2026-08-24: MEASURE 를 **기본 끔**으로 둔다.
-- 이 측정이 답해야 할 것(§3 열쇠)은 이미 답이 나왔고 -- (sector, 끝주소) --
-- 열쇠가 맞는지는 이제 **화면에 옳은 자막이 뜨는가**로 확인된다.  측정을 위해
-- 재생 중인 ADPCM RAM 을 512 회 읽는 것이 멈춤의 제일 유력한 원인이므로,
-- 얻을 것이 없는 위험을 기본으로 켜 둘 이유가 없다.
-- 다시 재야 할 일이 생기면 true 로 돌리면 된다.
local MEASURE  = false
local BACKSTOP = true
...
local KEYPATH = 'C:/snatcher/dump/probe_key_0_1_12_' .. STAMP .. '.tsv'
local CDDAPATH = 'C:/snatcher/dump/probe_cdda_0_1_12_' .. STAMP .. '.tsv'
...
-- satb 는 위에서 미리 선언했다.  여기서 다시 local 로 잡으면 안 된다.
local satb_decided = false

sat_field = function(c, slot, index)
...
if ready then
  ...
  for i = 0, P.adpcm_count - 1 do
    local at = P.adpcm_off + i * P.adpcm_stride
    local key = string.format('sec %06X 끝 %04X', u24(at), u16(at + 3))
...
emu.addEventCallback(function()
  ...
  -- ---- CD-DA 자막 (0.1.13) --------------------------------------------
  -- 음성 중에는 안 본다.  둘이 동시에 뜨면 백업이 서로를 덮는다.
  if not playing then
    local sec = s['cdrom.audioPlayer.currentSector']
    local rec = nil
    if type(sec) == 'number' then
      rec = find_cdda(math.floor(sec))
    end
    if rec ~= cdda_rec then
      if rec == nil then
        if cdda_rec ~= nil and armed then
          disarm()
          say(string.format('    CD-DA 자막 끝 (%d 구간 지나감)', cdda_shown))
        end
        cdda_rec = nil
      else
        local record = read_record(rec)
        if cdda_rec == nil then
          stage(record)                 -- 첫 구간: 자리를 빌린다
        else
          put_part(record)              -- 이어지는 구간: 글자만 바꾼다
        end
        cdda_rec = rec
        cdda_shown = cdda_shown + 1
        say(string.format('[%d] CD-DA 섹터 %d  y=%d  "%s"',
              frames, math.floor(sec), record.y, record.text))
      end
    end
  elseif cdda_rec ~= nil then
    -- 음성이 끼어들었다.  CD-DA 것을 먼저 걷는다.
  ...
  if playing and not was_playing then
    ...
    local found = find_parts(sector, ending)
    if #found == 0 then
      skipped = skipped + 1                -- 자막이 없다 = 효과음이거나 아직 번역 전
      local key = string.format('sec %06X 끝 %04X  (len %04X)', sector, ending,
            math.floor(s['cdrom.adpcm.adpcmLength'] or 0))
      ...
      say(string.format('[%d] sec %06X 끝 %04X  조각 %d개  y=%d  "%s"',
            frames, sector, ending, #found, record.y, record.text))
    end
  ...
  elseif not playing and was_playing then
    if armed then
      disarm()
      say(string.format('    음성 끝 -- VRAM %d 워드 · RAM %d B ($%04X-$%04X) 되돌림',
            VRAM_WORDS, #engine, ENGINE_AT, ENGINE_AT + #engine - 1))
    end
    parts = nil
  end
  was_playing = playing
  ...
  if frames % 3600 == 0 then
    say(string.format('--- %d --- 음성자막 %d · 넘김 %d · CD-DA자막 %d · 푸시 %d · 잔류지움 %d',
          frames, shown, skipped, cdda_shown, push_hits, cleared_total))
    flush()
  end
...
emu.addEventCallback(function()
  ...
  say(string.format('== 빌린 자리 == RAM %d 회 되돌림 (음성 수와 같아야 한다)',
        ram_restored))
  if ram_backup ~= nil then
    say('  ★ 떠 둔 RAM 이 남아 있다 -- 음성이 안 끝난 채로 Stop 했다')
  end
  if push_hits == 0 then
...
emu.log('PROBE_SUB_PACK 0.1.13 loaded  --  ADPCM + CD-DA 자막')
emu.log(string.format('  CD-DA 색인 %d 구간 (팩에 이미 있던 것) -- 이제 읽는다',
        P.cdda_count or 0))
emu.log(string.format('  MEASURE %s · BACKSTOP %s   (멈춤을 가르려면 하나씩 false 로)',
        MEASURE and '켬' or '★껐음', BACKSTOP and '켬' or '★껐음'))
