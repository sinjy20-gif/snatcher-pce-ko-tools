-- CDDA_SUBTITLE_POC 0.1.2 -- 현재 상주 훅 위에서 하는 첫 CD-DA 시각 렌더 실증.
--
...
emu.addEventCallback(function()
  ...
  if ok and s then
    ...
    if active == nil then
      ...
      local parts = seg and seg.clip == CDDA_TARGET_CLIP and TEST_PARTS or nil
      if parts == nil then
  ...
  if frames % 300 == 0 then
    say(string.format('--- %d --- 무장 %d · 푸시 %d · 지움 %d · 회수실패 %d · 본 지문 %d (카탈로그 밖 %d)',
          frames, arms, pushes, cleared, stranded, (function()
...
emu.log('CDDA_SUBTITLE_POC 0.1.2 loaded -- c17_001 only · resident-hook compatible')
