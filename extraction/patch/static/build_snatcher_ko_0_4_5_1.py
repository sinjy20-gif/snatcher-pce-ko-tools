...
PROTECTED = {VERSION, "0.4.5", "0.3.12-bios", "0.3.11-bios", "0.3.9-unpatched", "0.3.8.7", "0.3.8.6", "0.3.8.5", "0.3.8.4", "0.3.8.3", "0.3.8.2", "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
...
def check_inputs() -> None:
    ...
    # Runtime collection is the authority for dialogue window boundaries.
    # Do not build a disc when MASTER has dropped a tail row or split one
    # record_seq window across multiple text keys.
    coverage = subprocess.run(
        [sys.executable, str(ROOT / "tools" / "validate_runtime_window_coverage.py")],
        cwd=str(ROOT),
    )
    if coverage.returncode != 0:
        raise SystemExit("runtime window coverage check failed")
