#!/usr/bin/env python3
"""Snatcher Korean patch — 0.3.8.4.  타이틀 wipe 판정에서 상태를 없앤 판.
...
대조군이고, 실제로 플레이할 디스크는 그 다음이다.
    ...
    python build_snatcher_ko_0_3_8_4.py
    python patch_title_cache_wipe.py --source build\patch\0.3.8.4 --tag skip2
...
TSVs alone:
    ...
    python build_snatcher_ko_0_3_8_4.py              full build (stages 1-3)
    python build_snatcher_ko_0_3_8_4.py --stage3     stage 3 only, reusing stage 1/2
    python build_snatcher_ko_0_3_8_4.py --check      validate inputs, build nothing
...
revert:
    ...
    SNATCHER_PRELOAD_PACKS=0 python build_snatcher_ko_0_3_8_4.py
...
DEFAULTS = {
    ...
    "SNATCHER_TITLE_WIPE_GATE": "header",  # ask the cache header -- no state to lose
...
PROTECTED = {VERSION, "0.3.8.3", "0.3.8.2", "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
...
def main() -> None:
    ...
    # `SNATCHER_PRELOAD_PACKS=0 python build_snatcher_ko_0_3_8_4.py` still works
