#!/usr/bin/env python3
"""Build ac_0.1.4: safe first-render preload with a 16 KiB AC window."""
...
PRELOAD_HELPER = 0x9CD2  # Bank $69 mapped into MPR4 ($8000-$9FFF)
...
def emit_read_args(a: common.Assembler, length: int, current_lo: int, current_hi: int) -> None:
    for zp, value in ((0xF8, length), (0xF9, length >> 8), (0xFA, 0), (0xFB, 0x40), (0xFC, 0)):
        a.emit(0xA9, value, 0x85, zp)
...
def build_preload_helper() -> bytes:
    ...
    # Execute from MPR4/Bank69 while paused MPR2-3 become a 16 KiB AC window.
    a.emit(0x43, 0x04, 0x48, 0x43, 0x08, 0x48)
    a.emit(0xA9, 0x40, 0x53, 0x0C)
    ...
    a.emit(0x18); a.abs(0xAD, current_lo); a.emit(0x69, 8); a.abs(0x8D, current_lo)
    ...
    a.label("restore")
    a.abs(0x8D, status)
    a.emit(0x68, 0x53, 0x08, 0x68, 0x53, 0x04)  # restore MPR3/MPR2
...
def build_one_shot_wrapper() -> bytes:
    ...
    a.emit(0x43, 0x10, 0x48, 0xA9, 0x69, 0x53, 0x10)  # map Bank69 in MPR4
    a.abs(0x20, PRELOAD_HELPER)
    a.emit(0x85, 0x05, 0x68, 0x53, 0x10)  # status; restore MPR4
    a.emit(0xA5, 0x05)
    a.branch(0xD0, "restore")
    # Self-remove: future renderer entries call the proven $5E40 directly.
    a.emit(0xA9, 0x40); a.abs(0x8D, RENDERER_ENTRY + 1)
    a.emit(0xA9, 0x5E); a.abs(0x8D, RENDERER_ENTRY + 2)
    a.label("restore")
    a.emit(0xA2, 0)
...
def main() -> None:
    ...
    patches = [
        common.Patch("renderer_one_shot_hook", common.bank6a_iso(RENDERER_ENTRY), bytes.fromhex("20 40 5E"), bytes((0x20, 0xB3, 0x5B)), RENDERER_ENTRY),
        common.Patch("one_shot_preload_wrapper", common.bank68_iso(ONE_SHOT_WRAPPER), bytes((0xFF,)) * len(wrapper), wrapper, ONE_SHOT_WRAPPER),
        common.Patch("bank69_cd_to_ac_preloader", bank69_iso(PRELOAD_HELPER), bytes((0xFF,)) * len(helper), helper, PRELOAD_HELPER),
    ]
    ...
    manifest = {
        "version": VERSION,
        "base_build": str(BASE),
        "preload": "safe first-render one-shot Track-24 CD to Arcade Card RAM",
    ...
    (OUT / "TEST_IN_MESEN.txt").write_text(
        ...
        "The first BODY/UI Korean request performs the one-time 787840-byte preload.\n"
        "Wait for that initial load, then verify reception BODY/UI and absence of later Korean CD stalls.\n",
        encoding="utf-8-sig",
    )
