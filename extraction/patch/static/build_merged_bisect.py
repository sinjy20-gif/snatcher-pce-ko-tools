...
OUT = ROOT / "build" / "patch" / "MERGED BISECT"

RAW, HDR, USER = 2352, 16, 2048
B68, B6A = 0x07B800, 0x07F800

COMMON_R, COMMON_F = 0x5FF2, 0x5FF5
CANARY_AT = 0x5FF8
...
def main() -> None:
    ...
    print(f"0.2.26 의 $5FF2-$5FFF = {' '.join('%02X' % b for b in base)}")
    if set(base) != {0xFF}:
        print("  !! 경고: FF 가 아니다")

    build("bisect-common", False, False)
    build("bisect-render", True, False)
    build("bisect-font", False, True)
