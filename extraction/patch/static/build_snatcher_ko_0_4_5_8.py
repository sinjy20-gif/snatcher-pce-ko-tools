#!/usr/bin/env python3
"""Snatcher Korean patch - 0.4.5.8.  정식 빌드.

2026-08-24 저녁 수집용.  **엔진 변경 없음 -- 따옴표 규칙 위반 3건만 고쳤다.**
0.4.5.7 은 이 셋을 포함한 채로 나갔다.

    R02472:1 · R06128:1   ‘!’ -> 「!」  (서양식 따옴표 대신 이미 쓰던 낫표로)
    R06703:1              “『OUTER HEAVEN』의 -> 『OUTER HEAVEN』의
                           R02958 (같은 원문, 이미 검토 완료) 과 대조해 확인.
                           앞에 붙은 전각 여는따옴표는 2026-08-19 사고와 같은 패턴
                           (check_master.py 의 [quotes] 규칙 주석 참고).

check_master.py: 따옴표 0건 (반영 확인).

★ 배정표는 0.4.5.7 에서 이미 1,099 자로 확장했다 (--trust-observation, 재배정 0건).
  이 판은 배정표를 다시 안 건드린다.

★ 아직 남은 것 (이 빌드에서도 손대지 않았다):
  text_key 가 빠진 것으로 보이는 행 3건 (snatcher_tool (2) 에서 들어온 것).
  다음 검토 때 원본을 다시 봐야 한다.

    python build_snatcher_ko_0_4_5_8.py --check
    python build_snatcher_ko_0_4_5_8.py
    python patch_title_cache_wipe.py --source build/patch/EXPERIMENT/0.4.5.8-unpatched --out build/patch/0.4.5.8

**SNATCHER_KO_BIOS=1 을 빠뜨리면 안 된다.**  정상 지표는 631 B / 151 B 다.

--- 0.4.5.7 원문 ---

2026-08-24 낮 수집용.  **엔진 변경 없음 -- 번역 데이터만 늘었다.**
snatcher_tool (2) 에서 낮 동안 계속 검토한 마스터를 main 으로 병합해 반영한다
(0.4.5.6 이후 748 행 재검토/재랩 · master 전용 신규 3 행).

    python build_snatcher_ko_0_4_5_7.py --check   먼저 반영량만 본다
    python build_snatcher_ko_0_4_5_7.py
    python patch_title_cache_wipe.py --source build/patch/EXPERIMENT/0.4.5.7-unpatched --out build/patch/0.4.5.7

**SNATCHER_KO_BIOS=1 을 빠뜨리면 안 된다.**  정상 지표는 631 B / 151 B 다.

★ 글리프 배정표는 이 판에서도 갱신하지 않는다 -- 0.4.5.6 의 판단(2026-08-21)을
  그대로 따른다.  check_master.py 가 미배정 124 자를 보고했지만, 배정표를 다시
  돌리면 기존 배정이 흔들려 이미 나간 판과 어긋난다.  미배정 글자를 쓴 행은
  원문으로 남는다.

★ check_master.py 가 그 밖에 알린 것 (이 빌드에서 손대지 않았다):
  따옴표(‘’/“) 3건 · text_key 가 빠진 것으로 보이는 행 3건.  다음 검토 때 볼 것.

--- 0.4.5.6 원문 ---

2026-08-21 저녁 수집용.  **엔진 변경 없음 -- 번역 데이터만 늘었다.**
검토 5,404 / 6,075 행 기준 (0.4.5.5 는 4,102 행이었다.  +1,302).
...
★ 글리프 배정표는 이 판에서 갱신하지 않는다 (2026-08-21).
  `build_bios_hangul_map.py` 가 "빈 칸이 모자란다 -- 31 자 배정 못 함" 으로 멈춘다.
  게임이 쓴다고 판정된 글리프 1,906 중 런타임 관측은 1 개뿐이고 나머지는 코퍼스
  추정이다.  추정을 관측으로 바꾸기 전에는 자리가 안 난다.  배정표를 억지로 다시
  돌리면 기존 배정 10 자가 재배정되며 이미 나간 판과 어긋난다.
  -> 940 자 배정표를 그대로 쓴다.  미배정 글자를 쓴 행은 원문으로 남는다.

--- 0.4.5.5 원문 ---

2026-08-20 저녁 수집용.  **엔진 변경 없음 -- 번역 데이터만 늘었다.**
검토 완료 4,102 행 기준 (0.4.5.4 는 4,073 행이었다).

이 판을 만든 이유는 하나다: 환경 A에서 다음 수집을 돌릴 때 화면에 최신 번역이
떠 있어야 아직 안 된 곳이 눈에 보인다.

    python build_snatcher_ko_0_4_5_5.py
    python patch_title_cache_wipe.py --source build/patch/EXPERIMENT/0.4.5.5-unpatched                                      --out build/patch/0.4.5.5

**SNATCHER_KO_BIOS=1 을 빠뜨리면 안 된다.**  없으면 옛 비-BIOS 경로가 나와서
헬퍼가 777 B / 여유 5 B 로 보인다.  정상은 631 B / 151 B 다.

**BIOS 를 먼저 갱신할 것.**  배정표는 검토 O 인 행만 세므로, 검토가 늘었으면
새 글자가 생겼을 수 있다.  폰트 패치는 원본이 아니라 **현재 BIOS** 위에 얹는다 --
원본에서 새로 만들면 폰트 밖 패치(부팅 로고) 2,808 B 가 날아간다.

--- 0.4.5.1 원문 ---

Snatcher Korean patch - 0.4.5.1.  정식 빌드.

가우디 사전 755 행이 들어간 판 (2026-08-19).  엔진 변경 없음 -- 번역 데이터만
늘었다.  사전은 줄당 14 칸 · 문단 줄수 고정이라 대사(18 칸)와 규칙이 다르다.

    사전 범위   R01406~R02084 (용어·사건 해설) · R02112~R02116 (인물·사물)
    글리프      배정표 876 자 · 남은 빈 칸 90

**BIOS 를 먼저 갱신하고 빌드해야 한다.**  배정표는 검토 O 인 행만 세므로 검토가
끝난 뒤에 다시 돌려야 새 글자가 잡힌다.  폰트 패치는 원본이 아니라 **현재 BIOS**
위에 얹는다 -- 원본에서 새로 만들면 폰트 밖 패치(부팅 로고) 2,808 B 가 날아간다.

--- 0.4.5 원문 ---

Snatcher Korean patch - 0.4.5.  정식 빌드.

**0.3.12-bios 와 같은 엔진이다.  번호만 올렸다** (소유자 지시, 2026-08-19).
환경 A 작업분을 환경 B 트리로 되살리는 과정에서 폴더는 0.4.5, 매니페스트는
0.3.12-bios 로 어긋나 있던 것을 하나로 맞춘 것이다.  엔진 변경은 없다.

    build_snatcher_ko_0_3_12_bios.py   같은 코드.  옛 번호로 남겨둔다
    build_snatcher_ko_0_4_5.py         이것이 정식 진입점

    set SNATCHER_KO_BIOS=1
    python build_snatcher_ko_0_4_5.py
    python patch_title_cache_wipe.py --source <OUT> --out build/patch/0.4.5
...
PROTECTED = {VERSION, "0.4.5.5", "0.4.5.4", "0.4.5.3", "0.4.5.1", "0.4.5", "0.3.12-bios", "0.3.11-bios", "0.3.9-unpatched", "0.3.8.7", "0.3.8.6", "0.3.8.5", "0.3.8.4", "0.3.8.3", "0.3.8.2", "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
