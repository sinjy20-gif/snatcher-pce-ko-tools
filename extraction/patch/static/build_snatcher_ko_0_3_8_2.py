#!/usr/bin/env python3
"""Snatcher Korean patch — 0.3.8.2.  0.3.8.1 + 타이틀 캐시 wipe 스텁 (skip2).

이 빌드 자체는 스텁을 **엔진 이미지에 넣기만 한다.**  장면 VM 핸들러 표 $7331 을
스텁으로 돌리는 것은 별도 단계다.  그래서 이 폴더는 "스텁은 있으나 불리지 않는"
대조군이고, 실제로 플레이할 디스크는 그 다음이다.

    python build_snatcher_ko_0_3_8_2.py
    python patch_title_cache_wipe.py --source build\patch\0.3.8.2 --tag skip2
...
TSVs alone:
    ...
    python build_snatcher_ko_0_3_8_2.py              full build (stages 1-3)
    python build_snatcher_ko_0_3_8_2.py --stage3     stage 3 only, reusing stage 1/2
    python build_snatcher_ko_0_3_8_2.py --check      validate inputs, build nothing
...
revert:
    ...
    SNATCHER_PRELOAD_PACKS=0 python build_snatcher_ko_0_3_8_2.py
...
DEFAULTS = {
    ...
    "SNATCHER_TITLE_WIPE_SKIP": "2", # boot issues the title spawn twice -- skip2
...
PROTECTED = {VERSION, "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
...
def main() -> None:
    ...
    # `SNATCHER_PRELOAD_PACKS=0 python build_snatcher_ko_0_3_8_2.py` still works
