#!/usr/bin/env python3
"""Snatcher Korean patch - 0.4.5.7.  정식 빌드.

2026-08-24 번역 완료 수집용.  **엔진 변경 없음 -- 번역 데이터만 늘었다.**
소유자 확인: 남은 미검토 행은 게임에 안 들어가도 되는 것들이다 -- 번역은 사실상 완료.
빌드 시점 `--check` 기준: BODY reviewed 13,653 행 · UI translated 990/991 · UI review=O 982.
...
★ 글리프 배정표는 이 판에서도 갱신하지 않는다 (0.4.5.6 의 이유 그대로 유효).
...
PROTECTED = {VERSION, "0.4.5.6", "0.4.5.5", "0.4.5.4", "0.4.5.3", "0.4.5.1", "0.4.5", "0.3.12-bios", "0.3.11-bios", "0.3.9-unpatched", "0.3.8.7", "0.3.8.6", "0.3.8.5", "0.3.8.4", "0.3.8.3", "0.3.8.2", "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
...
def main() -> None:
    ...
    # ---- 장소(pack) 단위로 씬 팩을 나눈다 -- 이 판만의 차이다 --------------
    # 원래 classify 는 runtime_text_catalog 를 보는데 그 카탈로그가 보관소로
    # 안 옮겨져서 씬 배정이 0 행이었다.  그래서 전부 `runtime` 한 팩(936 KB)이
    # 되고, 첫 조회 자리에서 1,033 KB 를 한꺼번에 붓는다 -- 32 초.
    # 마스터의 `pack` 열이 곧 장소이므로 그것으로 직접 나눈다.
    import build_ac_dynamic_0_1_11_packs as packs
    # ⚠ packs -> build_ac_dynamic_0_1_11 이 임포트되면서 모듈 수준에서
    # `dynamic.BASE / OUT / VERSION` 을 자기 상수(ac_0.1.11-source)로
    # 되돌린다.  그래서 **임포트 뒤에 다시 세운다.**
    dynamic.VERSION = VERSION
    dynamic.OUT = OUT
    dynamic.BASE = STAGE1_BUILD
    dynamic.classify = packs.classify_by_pack
    if packs.SHARED_PACK not in dynamic.RESIDENT_PACK_NAMES:
        dynamic.RESIDENT_PACK_NAMES = (
            list(dynamic.RESIDENT_PACK_NAMES) + [packs.SHARED_PACK])
    print("  ★ 장소 단위 씬 팩으로 나눈다 (build_ac_dynamic_0_1_11_packs)")

    stage3.main()

    print()
    print("== 팩 나누기 ==")
    for name, n in packs._STATS.items():
        print(f"   {name:14} {n:6,}")
