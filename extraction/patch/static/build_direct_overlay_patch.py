...
LAYOUT_DIR = ROOT / "build" / "translation" / "current" / "runtime_layout_direct"
...
def build_renderer_preloader_direct(
    ...
    # Only narrative/speaker strings in the completed $3619 buffer are routed.
    # The $3499 path is action-menu/UI rendering.  UI localization is currently
    # disabled, so accepting that pointer would still hash and CD-read every
    # menu item despite leaving its Japanese text untouched.
    a.abs(0xAD, 0x3471)
    a.emit(0x85, 0x03)
    a.abs(0xAD, 0x3472)
    a.emit(0x85, 0x04)
    ...
    a.emit(0xA5, 0x04, 0xC9, 0x36)
    a.branch(0xF0, "pointer_ok")
    a.label("reject_pointer")
    a.abs(0x4C, "done")
    a.label("pointer_ok")

    # Seed root context from the captured MPR6 pack.  Search failure already
    # clears state and retries the legacy root, preserving every unmarked row.
    a.abs(0xAD, STATE_LO)
    a.abs(0x0D, STATE_HI)
...
def build(version: str, force: bool, review_only: bool = False) -> Path:
    ...
    layout_manifest = layout.build(
        STUDIO_TRANSLATION_DIR / "snatcher_ko_master.tsv",
        ...
        # Menus use a rapid redraw/caching path.  Leave them in Japanese until
        # the dedicated UI atlas is stable; speakers and dialogue stay active.
        include_ui=False,
    )
