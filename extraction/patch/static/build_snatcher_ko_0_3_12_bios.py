...
# 폴더는 0.4.x 인데 트랙·큐 파일 이름은 이 상수가 박혀 `[KO 0.3.12-bios]` 로
# 나갔다.  둘이 어긋나면 어느 디스크가 어느 빌드인지 헷갈린다 (소유자 지적,
# 2026-08-19 -- 실제로 0.4.2 를 0.4.5 인 줄 알고 돌려서 반나절을 태웠다).
#
# 그래서 출하 이름을 밖에서 넘길 수 있게 한다.  2 단계(patch_title_cache_wipe)는
# 소스 폴더의 이름을 그대로 따라가므로 여기만 맞추면 끝까지 일관된다.
#
#     SNATCHER_KO_VERSION=0.4.5.2 python build_snatcher_ko_0_3_12_bios.py
#
# 안 넘기면 예전 그대로다 -- 옛 빌드 재현이 깨지지 않는다.
VERSION = os.environ.get("SNATCHER_KO_VERSION", VERSION).strip() or VERSION

# 자막 POC 는 이름을 갈라 둔다.  0.3.12-bios 와 나란히 놓고 "바뀐 게 맞나" 를
# 눈으로 확인할 수 있어야 한다 -- 폴더와 트랙 파일 이름에 그대로 박힌다.
# 반복 시험 중에는 아래 숫자를 올린다.  큐 파일에는 시각(HHMM)도 붙는다.
if os.environ.get("SNATCHER_SUBS_POC", "0").strip() == "1":
    VERSION = "0.3.13-bios-poc"
...
DEFAULTS = {
    ...
    # 2026-08-20 추가.  전에는 환경변수로 손수 넣어야 했고 빠뜨리면 옛 비-BIOS
    # 경로가 나왔다 -- 15 자 예산이 되살아나 stage 1 이 멈추거나(운 좋은 경우),
    # 헬퍼가 777 B / 여유 5 B 인 다른 바이너리가 조용히 만들어졌다.
    # 반출본 README 가 "빠뜨리면 안 된다" 고 경고하던 것을 기본값으로 없앤다.
    # 정상 지표: helper code 631 B (151 B free).
    "SNATCHER_KO_BIOS": "1",
}
...
PROTECTED = {VERSION, "0.3.11-bios", "0.3.9-unpatched", "0.3.8.7", "0.3.8.6", "0.3.8.5", "0.3.8.4", "0.3.8.3", "0.3.8.2", "0.3.8.1", "0.3.8", "0.3.7", "0.3.6", "0.3.5", "0.3.4", "0.3.3", STAGE1_VERSION, "ac_0.1.14", "0.2.26"}
