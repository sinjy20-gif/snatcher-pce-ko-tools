...
TSVs alone:
    ...
    python build_snatcher_ko_0_3_3.py              full build (stages 1-3)
    python build_snatcher_ko_0_3_3.py --stage3     stage 3 only, reusing stage 1/2
    python build_snatcher_ko_0_3_3.py --check      validate inputs, build nothing
...
# other 22 tracks are hardlinks into rom(japan) and cost nothing.  Records --
# manifests, TSVs, verification logs -- total under 5 MB across every build ever
# made and are what makes an old build auditable, so pruning drops payload only.
#
# These names are refused even when named explicitly.  0.2.26 is the BODY
# control build for the save/overlay and abandoned-factory regressions and its
# cue is unusable without the referenced Track 02/24 payloads; the others are
# what the current build stands on.
...
def prune(names: list[str], deep: bool = False) -> None:
    ...
    2026-08-14: an allowlist replaced the old "everything except PROTECTED"
    sweep.  Standing rule from the project owner -- **no build is ever pruned
    unless he has called that build an experiment.**  A denylist meant every new
    release folder was prunable the moment the next one appeared, and the
    abandoned-factory investigation depends on old discs still being playable.
    The caller now has to name what goes, so nothing goes by default.

    ``deep`` additionally drops the stage 1/2 intermediate, which costs the next
    build its ``--stage3`` shortcut but reclaims another ~240 MB.
    """
    targets = set(names) | ({STAGE1_VERSION} if deep else set())
    if not targets:
        raise SystemExit(
            "--prune needs the build folders to drop, e.g.\n"
            f"    --prune {VERSION}-base {VERSION}-irqsplit\n"
