#!/usr/bin/env python3
"""Build ac_0.1.2: ac_0.1.1 runtime plus one-time in-game CD->AC preload."""
...
def build_preload_helper() -> bytes:
    ...
    # Execute from MPR4/Bank69 while MPR2 becomes the AC write window.
    a.emit(0x43, 0x04, 0x48, 0xA9, 0x40, 0x53, 0x04)  # TMA/PHA; LDA; TAM
    ...
    a.emit(0x18); a.abs(0xAD, current_lo); a.emit(0x69, 4); a.abs(0x8D, current_lo)
    ...
    a.label("restore")
    a.abs(0x8D, status)
    a.emit(0x68, 0x53, 0x04)  # restore MPR2
...
def main() -> None:
    ...
    manifest = {
        "version": VERSION,
        "base_build": str(BASE),
        "preload": "in-game one-shot Track-24 CD to Arcade Card RAM",
